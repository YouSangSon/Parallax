...
> 마지막 업데이트: 2026-05-10 (search retrieval depth v0 landed; next is budget/diversification/resource pagination/supersession)
...
| landed | `impact_trace_search_context` retrieval depth v0 — natural-language query는 read-only temp FTS5/BM25 keyword lane, 기존 `fact_embeddings`는 `semanticRank`, matched seed의 1-hop relation neighbor는 `graphProximityRank`로 RRF에 fuse |
...
| active next | persistent FTS5/BM25 projection for `relation_evidence`/selected `facts` and large-index search caps |
| active next | path/entity/relation diversification + returned bytes / estimated token budget |
| active next | persisted context pack id / repeated-query reuse |
| active next | paginated report/entity/evidence/graph resources |
| active next | typed error envelope (problem / cause / fix / evidence id) |
| active next | explicit supersession lifecycle for memory facts |
| after resource contract | UI Explorer v0 over the same MCP resource shapes |
| later | workspace diff tool — multi-repo 변경 + downstream consumer risk |
...
Phase 4 code baseline은 `33c49f0`에서 **112 tests passing**, ADR D-001..D-018, MCP 12개, CLI 16개였다. 현재 작업 브랜치는 session import v0와 agentmemory follow-up roadmap을 포함하며, 2026-05-10 기준 **191 tests passing**이다.
