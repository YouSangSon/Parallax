...
- `--sarif-output <path>` — GitHub Code Scanning upload용 SARIF 2.1.0 projection을 예쁘게 포맷한 JSON 파일로 쓴다. projection에는 affected-file finding과 recommended verification-action note가 포함된다. 부모 디렉터리는 생성된다. stdout에는 일반 human summary를 유지하며 `--json`과 함께 쓸 수 없다.
