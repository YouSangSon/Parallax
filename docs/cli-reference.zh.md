...
- `--sarif-output <path>` — 将 SARIF 2.1.0 projection 以格式化 JSON 写入文件，用于 GitHub Code Scanning upload。projection 包含 affected-file finding、cross-repo contract-break warning、recommended verification-action note 与 adapter known-gap note。父目录会自动创建。stdout 仍保留普通 human summary，且不能与 `--json` 同用。
