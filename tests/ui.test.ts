...
test('UI snapshot and HTML render a list-first report workbench', async () => {
  ...
  try {
    ...
    assert.match(html, /aria-label="Share and export"[\s\S]*id="copyLinkButton"[\s\S]*id="exportJsonButton"[\s\S]*id="exportCsvButton"[\s\S]*id="exportPngButton"/);
    assert.match(html, /\.export-controls \{[\s\S]*grid-template-columns: repeat\(4, minmax\(44px, max-content\)\);/);
    assert.match(html, /\.toolbar-action \{[\s\S]*min-height: 38px;[\s\S]*min-width: 44px;/);
    assert.match(html, /\.toolbar \{ min-width: 0; display: flex;/);
    assert.match(html, /\.copy-command \{[\s\S]*min-width: 44px;[\s\S]*min-height: 32px;/);
    assert.match(html, /function writeWorkbenchState\(updates, options = \{\}\)/);
    assert.match(html, /window\.history\.replaceState/);
    assert.match(html, /window\.history\.pushState/);
    assert.match(html, /function restoreWorkbenchState\(\)/);
    assert.match(html, /window\.addEventListener\('popstate', restoreWorkbenchState\)/);
    assert.match(html, /writeWorkbenchState\(\{ path \}\)/);
    assert.match(html, /writeWorkbenchState\(\{ filter: query \}, \{ replace: true \}\)/);
    assert.match(html, /\.evidence-row\.related-evidence, \.impact-path-main\.selected-impact/);
    assert.match(html, /function exportJson\(\)/);
    assert.match(html, /function exportCsv\(\)/);
    assert.match(html, /async function exportPng\(\)/);
    assert.match(html, /if \(!svg \|\| !svg\.querySelector\('\[data-impact-path\]'\)\) throw new Error\('no impact map to export'\);/);
    assert.match(html, /"exportDone":"Exported"/);
    assert.match(html, /wireToolbarButton\('exportJsonButton', exportJson, uiMessage\('exportDone', 'Exported'\)\)/);
    ...
    assert.match(html, /@media \(max-width: 560px\)[\s\S]*\.export-controls \{[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\);/);
    ...
    assert.match(html, /@media \(prefers-reduced-motion: reduce\) \{[\s\S]*scroll-behavior: auto !important;[\s\S]*transition-duration: 0\.01ms !important;/);
    assert.match(html, /function scrollBehavior\(\) \{[\s\S]*window\.matchMedia\?\.\('\(prefers-reduced-motion: reduce\)'\)\.matches \? 'auto' : 'smooth'/);
    assert.match(html, /\.selectable-impact:focus-visible \{[\s\S]*outline: 2px solid var\(--green\);/);
    assert.match(html, /\.impact-route-card\.selectable-impact:not\(\.selected-impact\):focus-visible \{[\s\S]*outline-color: var\(--ink-inverse\);/);
    assert.match(html, /\.delta-preset-select \{[\s\S]*font: inherit;[\s\S]*cursor: pointer;/);
    assert.match(html, /\.delta-path-select \{[\s\S]*font: inherit;[\s\S]*cursor: pointer;/);
    assert.doesNotMatch(html, /outline: 2px solid #73c2ac;/);
    ...
    assert.match(html, /<li class="impact-row impact-path-row" data-filter-text="[^"]+">[\s\S]*?<button class="impact-path-main selectable-impact" type="button" data-impact-path="src\/a\.ts"/);
    assert.doesNotMatch(html, /<li class="impact-row impact-path-row[^>]*(?:tabindex|role)=/);
...
test('UI snapshot and HTML compare the selected report to the previous saved report', async () => {
  ...
  try {
    ...
    assert.match(html, /<li class="delta-preset delta-preset-wider">[\s\S]*?<button class="delta-preset-select" type="button" data-policy-preset="strict">/);
    assert.doesNotMatch(html, /<li class="delta-preset[^>]*(?:tabindex|role)=/);
    assert.match(html, /function selectPreset\(preset, options = \{\}\)/);
    assert.match(html, /writeWorkbenchState\(\{ preset \}\)/);
    assert.match(html, /\.delta-preset\.selected-preset/);
    ...
    assert.match(html, /<li class="delta-path-row" data-filter-text="added impact README\.md">[\s\S]*?<button class="delta-path-select selectable-impact" type="button" data-impact-path="README\.md"/);
    assert.doesNotMatch(html, /<li class="delta-path-row[^>]*(?:tabindex|role)=/);
    assert.match(html, /Inspect impact/);
    assert.match(html, new RegExp(`/source\\?path=README\\.md&amp;line=1&amp;report=${reportId}&amp;lang=en`));
    assert.doesNotMatch(html, /document\.querySelectorAll\('\.selectable-impact a, \.selectable-impact button'\)/);
...
test('UI server exposes bootstrap and resource-shaped JSON endpoints', async () => {
  ...
  try {
    ...
    assert.match(htmlResponse.headers.get('content-security-policy') ?? '', /img-src 'self' blob: data:/);
