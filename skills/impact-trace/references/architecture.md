...
Implication: there is no in-place update of a fact. Updating a value means writing a *new* fact (whose id differs because the value differs) on a new transaction. This is decision D-002 in `docs/decisions.ko.md`.
...
- New behavior that changes invariants → write a new ADR in `docs/decisions.ko.md` first
