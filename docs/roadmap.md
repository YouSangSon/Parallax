...
> 마지막 업데이트: 2026-05-11 (UI Explorer v0 + TS/JS import spans + JVM/Spring spans + Python/Go/Rust spans landed; next is workspace contracts)
...
다음 live work는 [Phase 6B Multi-language + Spring Boot Adapter Pack v0 plan](phase6b-ts-accuracy-plan.ko.md)이다. 완료된 항목: ImpactBench thin spine, TS/JS/JVM-Spring/Python/Go/Rust v0 adapter routing, Spring Boot endpoint/config/persistence/test/client fixture coverage, adapter attribution scoring, TS/JS parser-backed import span v0, JVM/Spring lightweight evidence span v0, Python/Go/Rust lightweight evidence span v0. 아직 완료로 표시하지 않는 항목: workspace loader/resolver, full parser/LSP depth.
...
| active next | workspace diff tool / contract impact — multi-repo 변경 + downstream consumer risk |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-024)
