...
Repository checkout에는 TypeScript source file, test, benchmark fixture, Claude Code / Codex 사용자용 Parallax skill이 `skills/` 아래에 있다. npm package는 build된 CLI와 public docs를 싣지만 skill directory는 싣지 않으므로, packaged docs에서는 `skills/`로 링크하지 않는다. Architecture와 release checklist 같은 maintainer 문서는 source checkout이 필요한 경우를 본문에서 명시한다.
