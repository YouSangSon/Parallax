...
- [ ] 在 embedding 模型 / LLM provider 交叉时对 recall 质量的回归 detection
- [x] 在 CI 中每个 PR 自动报告 bench delta
  - 当前 gate：CI 会在 pull request 中准备 base SHA 的 bench report，在 head 上运行 canonical `npm run verify` gate，然后把 `npm run bench:report` Markdown append 到 GitHub Step Summary，展示 score、relation、affected-file 与 retrieval delta。
