...
| D7 | **SARIF / GitHub Code Scanning export** — ✅ **affected-file SARIF projection shipped** from `ImpactReport` via `parallax analyze --sarif-output <path> [--sarif-category <category>]`, plus evidence locations, relation paths, confidence rules, stable fingerprints, and docs for Code Scanning upload. ✅ **recommended verification actions** and ✅ **adapter `knownGaps`** now emit as SARIF note results. Still open: contract breaks and coverage gaps. | M | HIGH |
...
3. **D7 SARIF breadth** — extend Code Scanning projection beyond affected files to contract breaks and coverage gaps.
