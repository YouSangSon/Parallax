...
- `.impact-trace/workspace.json` 기반 local workspace catalog와 `workspace init/add-repo/list` CLI
- import 기반 관련 테스트 추론
...
- Obsidian write sync
- graph DB projection
- cross-repo contract resolver와 breaking-change 분류
- web graph explorer
- CodeQL adapter
...
누적 결정 로그: [decisions.ko.md (D-001..D-026)](docs/decisions.ko.md).
...
- [Phase 6B multi-language + Spring Boot 계획](docs/phase6b-ts-accuracy-plan.ko.md) — 현재 slice: adapter pack v0 routing, ImpactBench fixture, TS/JS parser-backed import span v0, JVM/Spring lightweight evidence span v0, Python/Go/Rust lightweight span v0, OpenAPI contract impact baseline, workspace catalog v0, 다음 depth pass: cross-repo resolver
- [Architecture decisions log (D-001..D-026)](docs/decisions.ko.md) — 누적 ADR 로그
...
5. parser-backed/lightweight adapter depth pass와 source-span evidence 확대: TS/JS import spans, JVM/Spring lightweight spans, Python/Go/Rust lightweight spans, OpenAPI contract baseline과 workspace catalog v0 landed, 다음은 cross-repo resolver depth
...
8. cross-repo API/gRPC/event contract impact 분석 추가
