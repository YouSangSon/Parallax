...
| 5 | contract diff impact | 진행 중. workspace catalog, contract baseline, cross-repo links 위에서 OpenAPI endpoint/nested schema risk, Protobuf RPC/message field risk, GraphQL root/object/input schema risk를 만든다. 다음은 AsyncAPI diff와 GraphQL consumer resolver다. |
