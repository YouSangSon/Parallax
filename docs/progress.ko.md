...
| Phase B | MCP retrieval depth + resource pagination | context/search/explain/telemetry/doctor/session import v0/v1, search retrieval depth v0, search budget/diversification v0는 완료. 다음 slice는 persistent FTS projection, retrieval bench, report/graph pagination |
...
1. relation_evidence/facts persistent FTS projection과 retrieval/context effectiveness bench를 추가해 Recall@budget, returned bytes, resource expansion rate를 측정한다.
2. `impact-trace://reports/{id}/graph/{format}` pagination과 typed error envelope를 구현한다.
3. explicit supersession으로 오래된 decision/summary/policy fact를 명시적으로 대체한다.
4. 같은 resource contract를 읽는 `impact-trace ui` workbench v0를 만든다.
