...
- docs/ADR 정리: Phase 6 foundation부터 UI work artifact freshness badge까지 decisions log D-045로 갱신
...
5. **UI work artifact impact/metadata/freshness surface v0:** selected report의 policy, decision, PRD, requirement, proposal impact가 `workArtifacts`와 Work Artifacts panel로 분리된다. Markdown frontmatter/문서 선두 H1에서 title/owner/status/updatedAt만 추출하고, stale/current/unknown freshness를 표시하되 본문은 싣지 않는다.
