...
> 마지막 업데이트: 2026-05-11 (UI Explorer v0 + TS/JS/JVM/Spring/Python/Go/Rust spans + OpenAPI contract impact baseline + workspace catalog v0 + cross-repo contract resolver v0 + OpenAPI contract diff v0 landed; next is schema/body-level contract diff and MCP workspace resources)
...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0, endpoint-surface contract diff v0는 landed. 다음은 schema/body-level diff와 protobuf/GraphQL/AsyncAPI 확장. |
...
| active next | MCP workspace/contract resources + schema/body diff — OpenAPI endpoint diff 결과를 resource로 노출하고 response/request schema breaking rules로 확장 |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-028)
