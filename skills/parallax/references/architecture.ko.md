...
`src/`는 책임 단위로 구성되어 있다. 이 맵은 각 관심사가 어디에 사는지를 보여주므로, 어떤 파일을 읽거나 확장해야 할지 알 수 있다.

**Entry & orchestration**

| Path | 책임 |
|---|---|
| `src/index.ts` | Public API barrel — 프로그래밍 방식으로 지원되는 표면을 re-export |
| `src/cli.ts` | CLI dispatch (커맨드 if-chain, 플래그 파싱, 도움말 텍스트) |
| `src/indexer.ts` | 인덱싱 파이프라인 + `createDefaultRegistry` (어댑터를 순서대로 연결) |
| `src/analyzer.ts` | 인덱싱된 그래프 위에서의 impact 계산 |
| `src/graph.ts` | 그래프 export |
| `src/store.ts` | SQLite 스키마, ADD-only 마이그레이션, DB 접근 헬퍼 |
| `src/types.ts` | 공유 타입 정의 |
| `src/confidence.ts` | 공유 confidence 가드 (`asConfidence` — 유효한 `Confidence`로 강제 변환) |

**MCP surface**

| Path | 책임 |
|---|---|
| `src/mcp.ts` | MCP 서버: tool/resource 등록 + context-pack 연결 |
| `src/mcp_search.ts` | 검색 서브시스템 — keyword/FTS/semantic/graph-proximity 랭킹 + 행 쿼리 |
| `src/mcp_shared.ts` | `mcp.ts`와 `mcp_search.ts`가 공유하는 DB 헬퍼 |
| `src/context_pack.ts` | Context-pack 빌드 / freshness / compaction 파이프라인 |

**Adapters** (`src/adapters/`)

| Path | 책임 |
|---|---|
| `src/adapters/registry.ts` | First-match-wins 디스패치, catch-all은 마지막 |
| `src/adapters/types.ts` | `SemanticAdapter` 계약 (descriptor, pending entity/relation/evidence) |
| `src/adapters/multi-language-regex.ts` | 폭넓은 multi-language catch-all 어댑터 |
| `src/adapters/config-infra.ts` | Config / infrastructure 파일 어댑터 |
| `src/adapters/build-system-package.ts` | Build-system / package-manifest 어댑터 진입점 |
| `src/adapters/build-system/{npm,maven,gradle,go,cargo,python}.ts` | 생태계별 manifest 파서 |
| `src/adapters/build-system/{shared,types}.ts` | 생태계 파서들이 공유하는 헬퍼와 타입 |

**Contracts**

| Path | 책임 |
|---|---|
| `src/contract_diff.ts` | 오케스트레이터 — 포맷별 분류기로 디스패치 |
| `src/contract_diff/{openapi,asyncapi,protobuf,graphql}.ts` | 포맷별 호환성 분류기 (diff 변경을 생성) |
| `src/contract_diff/{shared,types}.ts` | 분류기가 공유하는 헬퍼와 타입 |
| `src/{openapi,graphql,protobuf,asyncapi}_compat.ts` | 분류기가 소비하는 구조화된 호환성 시그니처를 추출하는 저수준 파서 |
...
| `src/reflection.ts` | Phase 3 LLM 통합 파이프라인 (summary fact) |
| `src/profile.ts` | Phase 4 집계된 entity 스냅샷 (recall 위에 빌드) |
| `src/branch_gc.ts` | Soft-delete branch GC (`gcBranches`) |
| `src/session_import.ts` | Codex/Claude 세션 트랜스크립트를 fact로 import |
| `src/embeddings.ts` | Embedding provider 추상화 (prefix-sentinel) |
| `src/llm.ts` | LLM provider 추상화 (`summarize`, prefix-sentinel) |

**Workspace & cross-repo**

| Path | 책임 |
|---|---|
| `src/workspace.ts` | Workspace 카탈로그 (이름 붙은 multi-repo 멤버십 + trust policy) |
| `src/cross_repo_resolver.ts` | Workspace 전반에서 cross-repo 계약 링크(예: `CONSUMES_HTTP_ENDPOINT`)를 해석 |

**UI** (`src/ui.ts` + `src/ui/`)

| Path | 책임 |
|---|---|
| `src/ui.ts` | UI 서버 + `renderUiHtml` 조합 + 스냅샷 빌드 |
| `src/ui/styles.ts`, `src/ui/client.ts` | 추출된 정적 CSS / 클라이언트 JS |
| `src/ui/impact_map.ts` | Impact-map 렌더링 |
| `src/ui/report_delta.ts` | Report-delta 정책 + 렌더링 |
| `src/ui/shared.ts` | 공유 UI 표현 헬퍼 |

**Supporting**

| Path | 책임 |
|---|---|
| `src/security.ts` | 시크릿 redaction + 경계에서의 path/root 정규화 |
| `src/artifacts.ts` | Work-artifact (markdown doc/policy/proposal/…) 분류 |
| `src/git-snapshot.ts` | Git commit/branch/dirty 스냅샷 캡처 |
| `src/doctor.ts` | 환경/헬스 진단 |
| `src/init.ts` | 프로젝트 초기화 (`.parallax/` config + DB) |
| `src/branding.ts` | 제품/패키지 이름, `.parallax` 데이터 디렉터리, `PARALLAX_*` env 리더 |
...
- New adapter → `src/adapters/types.ts`의 `SemanticAdapter` 계약을 구현하고 `src/indexer.ts:createDefaultRegistry`에 등록하라 (순서가 중요하다 — catch-all은 마지막에 둔다); multi-file 어댑터라면 `src/adapters/build-system/`을 따르라
