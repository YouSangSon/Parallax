...
- [x] A deterministic bench harness based on multi-language fixtures
  ...
  - Current contract-diff gate: the bench includes paired OpenAPI v1/v2 quality cases for removed response required properties, added request required properties, request enum-value removals, response property type changes, response nullable additions, response format changes, and response enum-value removals, reported as `contractDiffQuality` so CI summaries can track deltas.
