...
test('UI snapshot and HTML render a list-first report workbench', async () => {
  ...
  try {
    ...
    assert.match(html, /function replaceWorkbenchState\(updates\)/);
    assert.match(html, /window\.history\.replaceState/);
    assert.match(html, /replaceWorkbenchState\(\{ path \}\)/);
...
test('UI snapshot and HTML compare the selected report to the previous saved report', async () => {
  ...
  try {
    ...
    assert.match(html, /class="delta-preset delta-preset-wider" tabindex="0" role="button" data-policy-preset="strict"/);
    assert.match(html, /function selectPreset\(preset, options = \{\}\)/);
    assert.match(html, /replaceWorkbenchState\(\{ preset \}\)/);
