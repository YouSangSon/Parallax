...
package resolver는 npm lockfile을 쓰기 시작했지만, 대부분의 lockfile ecosystem과 semver 영향 분석은 아직 남아 있다.

- [ ] lockfile 기반 transitive 의존성 그래프 (npm·pip·poetry·go·cargo·maven·gradle)
  - 진행: npm `package-lock.json` v2/v3의 transitive package entry를 locked version과 evidence span이 있는 lockfile-derived `DEPENDS_ON` package relation으로 index한다.
