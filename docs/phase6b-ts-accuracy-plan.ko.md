...
4. **Build-system/package resolver v0:** `package.json`, `pom.xml`, `build.gradle(.kts)`, Gradle `libs.versions.toml`, `go.mod`, `Cargo.toml`, `pyproject.toml` manifest-only package graph가 landed 상태다. Gradle version catalog `[versions]`/`[libraries]`/`[bundles]` alias는 actual package coordinate와 version metadata로 펼치며, `platform(libs.*)`/`enforcedPlatform(libs.*)` 같은 nested accessor도 dependency type을 유지한다. package manager/build tool execution, lockfile/transitive resolution, Maven profiles, custom/imported Gradle catalogs, Go workspace/replace depth, Python optional dependency groups는 후속이다.
