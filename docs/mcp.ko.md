...
Parallax는 불변 원칙 **I-8**([invariants.ko.md](invariants.ko.md) 참고)을 따른다 — agent surface는 안전한 read-only 분석 계층을 먼저 안정화하고, 쓰기 권한은 별도 모델과 리뷰 뒤에만 추가한다. 각 tool은 MCP `readOnlyHint` 어노테이션을 선언한다. 다섯 개 tool이 `readOnlyHint: true`로 표시된다 — `parallax_recall`, `parallax_profile`, `parallax_context_telemetry`, `parallax_doctor`, `parallax_trace` — 이들은 순수 읽기다. 나머지 tool은 `readOnlyHint: false`다. 여기에는 분석 tool도 포함되는데, 응답하는 부수 효과로 context-pack과 telemetry 행을 저장하기 때문이며, 명시적 메모리 쓰기와 branch 관리 tool도 마찬가지다. 이들 중 어느 것도 소스 트리를 수정하지 않는다 — action은 추천일 뿐이다(불변 원칙 **I-9**).

## Tool

18개 tool 전부 `parallax_` 접두사로 등록된다. *read-only* 열은 tool의 MCP `readOnlyHint` 어노테이션을 반영한다.
