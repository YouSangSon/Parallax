...
| `parallax_cross_repo_consumers` | 从已持久化的 workspace link 查询某个 provider service/contract/route 的 consumer | 是 |
| `parallax_cross_repo_providers` | 从已持久化的 workspace link 查询某个 consumer service/file 使用的 provider | 是 |
| `parallax_resolve_cross_repo_contracts` | 预览 cross-repo provider/consumer contract link，但不持久化 workspace link row | 是 |
