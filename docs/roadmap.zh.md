...
- [ ] 将 lockfile-based transitive 依赖图扩展到 pip/poetry/go/cargo/maven/gradle lockfile
...
- [x] 只读图查询面：将确定性 Cypher 子集作为 `parallax_query` MCP 工具和 `parallax query` CLI 暴露（单 hop、label、`WHERE` =/`CONTAINS`、投影、`LIMIT`；拒绝 write/procedure/反向子句）
