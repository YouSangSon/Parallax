...
업데이트: 2026-05-10 (Phase 6B adapter pack v0 + agentmemory adoption review + MCP context telemetry v0)
...
| Phase B | MCP context pack/search/telemetry v0/v1 | `impact_trace_context_for_change`로 agent가 작업 전 compact context를 받는 첫 tool 구현. source/report는 변경하지 않고 repo-local telemetry만 append한다. evidence resource v0, explain entity v0, search context v0, search ranking v1, context telemetry v0는 완료, report/graph pagination은 다음 slice |
