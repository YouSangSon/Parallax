...
| Phase B | MCP retrieval depth + resource pagination | context/search/explain/telemetry/doctor/session import v0/v1, search retrieval depth v0, search budget/diversification v0, persistent FTS + retrieval bench v0는 완료. 다음 slice는 report/graph pagination과 typed errors |
...
1. `impact-trace://reports/{id}/graph/{format}` pagination과 typed error envelope를 구현한다.
