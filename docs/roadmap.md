...
> 마지막 업데이트: 2026-05-13 (MCP work artifact context preview + UI work artifact metadata/freshness preview + UI work artifact impact surface + UI workspace topology surface + TS/JS/JVM/Spring/Python/Go/Rust spans + OpenAPI contract impact baseline + workspace catalog v0 + cross-repo contract resolver v0 + OpenAPI HTTP route alias resolver v0 + GraphQL/Protobuf/AsyncAPI consumer resolver v0 + generated-client/event topology v0 + contract topology surface v0 + OpenAPI nested endpoint/schema diff v0 + Protobuf contract diff v0 + GraphQL contract diff v0 + AsyncAPI contract diff v0 + build-system/package resolver v0 + Gradle version catalog resolver v0 + Maven property interpolation v0 + Go workspace/replace resolver v0 + Python optional/dependency groups v0 + Cargo workspace/path resolver v0 + config/infra adapter v0 + MCP workspace/contract resources v0 landed; next is full parser/LSP depth, richer topology, and deeper package/build resolution)
...
| A4 | npm/pnpm/yarn + Maven/Gradle/Cargo/Go/Python workspace 어댑터 | v0 landed: `package.json`, `pom.xml` same-file property interpolation, `build.gradle(.kts)`, Gradle `libs.versions.toml` direct/nested accessor resolution, `go.mod`, `go.work` use directory, repo-local Go `replace`, `Cargo.toml` workspace/path dependency graph, `pyproject.toml` manifest-only package graph, Python optional/dependency groups. Cargo 실행, feature resolver, target cfg evaluation, `Cargo.lock`, `package.workspace` override, `[workspace].exclude`, complex glob, lockfile/transitive/semver/workspace/include expansion depth는 후속이다. |
| A5 | YAML / GitHub Actions / Docker / Terraform 어댑터 | 부분 완료. GitHub Actions workflow YAML, Dockerfile/Containerfile, Terraform `.tf`의 explicit repo-local path impact는 `config-infra-semantic-v0`로 regex fallback에서 분리됨. 다음은 generic YAML semantics, Docker build context/glob/multistage, Terraform module/provider graph. |
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0, OpenAPI same-file HTTP route alias resolver v0, GraphQL/Protobuf/AsyncAPI consumer resolver v0, generated-client/event topology v0, contract diff topology provenance, contract topology surface v0, endpoint-surface contract diff v0, JSON/YAML nested schema/body diff v0, Protobuf contract diff v0, GraphQL contract diff v0, AsyncAPI contract diff v0는 landed. 다음은 richer topology와 full parser depth. |
...
| landed | MCP work artifact context preview — `context_for_change`가 body-free `workArtifacts` lane으로 policy/decision/PRD/proposal impact와 metadata/freshness를 전달 |
...
| landed | UI workspace topology surface — `impact-trace ui` now previews workspace contract baselines, provider/consumer links, event topology hints, and `/api/workspaces/{name}` without requiring full provenance expansion |
| landed | UI work artifact impact surface — selected report의 policy/decision/PRD/requirement/proposal impact를 Work Artifacts panel과 `bootstrap.workArtifacts`로 분리 |
| landed | UI work artifact metadata preview — Markdown frontmatter/문서 선두 H1에서 title/owner/status/updatedAt을 추출해 Work Artifacts row에 compact metadata로 표시 |
| landed | UI work artifact freshness badge — `updatedAt`을 report 생성 시각 기준 stale/current/unknown으로 분류해 오래된 work artifact impact를 먼저 표시 |
| landed | MCP workspace/contract resources v0 — `impact_trace_contract_diff`, `impact-trace://workspaces/{name}`, `/contracts`, `/cross-repo-links`로 endpoint diff 결과와 provider/consumer links를 resource-on-demand로 확장. contract topology surface v0는 summary/CLI/resource에 producer/consumer hint를 compact field로 노출 |
| landed | build-system/package resolver v0 — manifest-only package graph를 context pack 입력으로 제공해 package manifest 변경의 downstream impact를 줄인 payload로 전달. Cargo는 실행, feature resolver, target cfg evaluation, `Cargo.lock`, `package.workspace` override, `[workspace].exclude`, complex glob, lockfile/transitive graph 없이 정적 manifest만 사용 |
| active next | full parser/LSP and resolver depth — cross-file generated-client data flow, richer event topology, lockfile/transitive package graph, language-server-backed references로 v0 heuristics를 보강 |
...
| UI/MCP Work Artifacts preview — v0 landed: selected report와 context pack에서 work artifact impact를 별도 panel/lane과 resource link preview로 노출하고 compact metadata/freshness를 표시 |
| frontmatter/heading metadata extraction — v0 landed: title/owner/status/updatedAt compact preview |
| stale decision/proposal freshness calculation — v0 landed: `updatedAt` 기반 stale/current/unknown preview |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-040)
