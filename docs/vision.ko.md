...
Local-first가 토대 — 모든 데이터는 `<repo>/.impact-trace/impact.db`. 외부 서비스 없음, cloud sync 없음, graph DB 필수 아님. 이유:
...
**3년차:** MemoryBench harness가 회귀 신호를 제공해 메모리 작업의 품질이 *측정 가능하게* 개선됨 (embedding 모델 / LLM provider / reflection 알고리즘 교차). UI, graph DB projection, IDE/plugin surfaces가 first-class consumer가 되고, impact-trace는 canonical SQLite source of truth로 유지.
