...
| `parallax_query` | Run a read-only Cypher subset (single hop, labels, WHERE =/CONTAINS, projection, LIMIT) over the indexed entity/relation graph and return JSON rows | Yes |
