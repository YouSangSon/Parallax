...
agent memory tool과 context telemetry는 현재 저장소의 `.impact-trace/impact.db`에만 씁니다.
...
| `impact_trace_merge` | 두 branch의 head를 묶어 새 merge 트랜잭션을 만듭니다. |
| `impact_trace_trace` | fact_provenance edge를 따라 결정의 인과 사슬과 edge kind(`evidence`/`summary`/`supersedes`)를 반환합니다. |
| `impact_trace_reflect` | 오래된 facts를 entity별로 LLM이 요약해 summary fact로 승격합니다. |
| `impact_trace_abandon_branch` | branch state를 `abandoned`로 변경합니다. main은 보호. |
| `impact_trace_gc_branches` | abandoned branch의 transactions를 soft-delete archive 처리합니다. `maxAgeDays`로 시간 기반 자동 abandon. |
| `impact_trace_profile` | 한 entity의 facts를 staticFacts/dynamicFacts/summaryFacts 3-bucket으로 한 번에 반환합니다. |
| `impact_trace_repair_reflections` | orphan summary fact를 보정합니다. |
| `impact_trace_restore_branch` | abandoned branch의 state + tx archived를 복구합니다. |
...
`impact_trace_contract_diff` v0는 CLI `workspace contract-diff`와 같은 contract classifier를 MCP로 노출합니다.
...
- 영향 분석/context MCP tool은 source/report를 변경하지 않습니다. 단, context 절감 측정을 위해
  repo-local `.impact-trace/impact.db`에 telemetry row를 best-effort로 append합니다.
...
| `npm run bench` | multi-language/Spring Boot/contract/package-manifest fixture를 인덱싱하고 deterministic JSON report를 `.impact-trace/bench/impact-bench-report.json`에 씁니다. |
...
- [skills/impact-trace/SKILL.md](skills/impact-trace/SKILL.md) — Claude Code/Codex 사용자용 스킬
