...
- docs/ADR 정리: Phase 6 foundation부터 UI work artifact metadata preview까지 decisions log D-044로 갱신
...
- Python virtualenv/package resolver, Go workspace resolver, Cargo feature resolver, package-manager lockfile/transitive resolver
...
3. **Workspace catalog + resolver + contract diff + MCP/UI resources v0:** `.impact-trace/workspace.json` local allowlist, `workspace init/add-repo/list/resolve-contracts/contract-diff` CLI, OpenAPI HTTP consumer links, GraphQL operation-document consumer links, Protobuf generated-client/full-route consumer links, AsyncAPI producer/consumer topology hints, contract topology summary/CLI/resource surface, JSON/YAML OpenAPI nested request/response schema breaking rules, Protobuf service/RPC/message field breaking rules, GraphQL root/object/input schema breaking rules, AsyncAPI operation/message payload breaking rules, `impact_trace_contract_diff`, `impact-trace://workspaces/{name}/contracts`, `/cross-repo-links` resource, `impact-trace ui` workspace topology panel, `/api/workspaces/{name}` JSON이 landed 상태다.
4. **Build-system/package resolver v0:** `package.json`, `pom.xml`, `build.gradle(.kts)`, `go.mod`, `Cargo.toml`, `pyproject.toml` manifest-only package graph가 landed 상태다. package manager/build tool execution, lockfile/transitive resolution, Maven profiles, Gradle version catalogs, Go workspace/replace depth, Python optional dependency groups는 후속이다.
5. **UI work artifact impact/metadata surface v0:** selected report의 policy, decision, PRD, requirement, proposal impact가 `workArtifacts`와 Work Artifacts panel로 분리된다. Markdown frontmatter/문서 선두 H1에서 title/owner/status/updatedAt만 추출하고 본문은 싣지 않는다.
