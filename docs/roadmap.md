...
| A4 | npm/pnpm/yarn + Maven/Gradle/Cargo/Go workspace 어댑터 | v0 landed: `package.json`, `pom.xml` same-file property interpolation, `build.gradle(.kts)`, Gradle `libs.versions.toml` direct/nested accessor resolution, `go.mod`, `go.work` use directory, repo-local Go `replace`, `Cargo.toml`, `pyproject.toml` manifest-only package graph. 다음은 lockfile/transitive/semver/workspace depth. |
