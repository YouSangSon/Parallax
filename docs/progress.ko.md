...
업데이트: 2026-05-10 (Phase 6B adapter pack v0 + agentmemory adoption review + session import v0)
...
| Phase B | MCP context pack/search/telemetry/doctor v0/v1 | `impact_trace_context_for_change`로 agent가 작업 전 compact context를 받는 첫 tool 구현. source/report는 변경하지 않고 repo-local telemetry만 append한다. evidence resource v0, explain entity v0, search context v0, search ranking v1, context telemetry v0, doctor v0는 완료, report/graph pagination은 다음 slice |
| Phase B | agentmemory-informed context lifecycle | [agentmemory 적용성 분석](agentmemory-adoption-review.ko.md)에 따라 explicit supersession, opt-in session import를 SQLite/provenance 경계 안에서 구현. RRF ranking initial slice, context telemetry v0, session import v0는 완료했고 FTS/BM25 + semantic recall depth pass는 후속 |
