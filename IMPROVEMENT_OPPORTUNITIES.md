...
| W4 | **Richer contract property signatures** — ✅ response enum-value removal and response format changes are now captured for OpenAPI via richer property signatures, compat schemaVersion 4, persisted breaking-change provenance, and `contractDiffQuality` bench cases. Remaining: `nullable`, request enum/format narrowing/broadening semantics, and response optionality rules. The substance of "nested-schema-level". | M | MED-HIGH |
...
**Sequencing remaining work:** W4 follow-ups (`nullable` / request enum and format semantics) then W5 JSON Schema reuse → W3 (biggest scope, monorepo users). W1/W2/W6 are already shipped.
