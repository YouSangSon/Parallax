...
test('TypeScript JavaScript adapter records parser-backed method reference alias call spans', async () => {
  const repoRoot = await mkdtemp(path.join(tmpdir(), 'parallax-ts-method-reference-alias-spans-'));
  await mkdir(path.join(repoRoot, 'src'), { recursive: true });
  ...
  await writeFile(path.join(repoRoot, 'src/guard.ts'), [
    ...
    '  const guard: SessionGuard = new SessionGuard();',
    '  const validate = guard.validateSession;',
    '  const audit = guard.auditSession.bind(guard);',
    '  return validate(token) && audit(token);',
    '}',
    ''
  ].join('\n'));
  ...
  try {
    const calls = db
      .prepare(`
        ...
        WHERE r.index_run_id = ?
          ...
          AND source.path = 'src/guard.ts'
          AND target.path = 'src/guard.ts'
          AND ev.start_line = 10
        ORDER BY ev.start_col
      `)
      .all(index.indexRunId) as Array<{
        source_symbol: string;
        target_symbol: string;
    ...
    assert.deepEqual(
      calls.map((row) => [row.source_symbol, row.target_symbol, row.snippet]),
      [
        ['authorize', 'SessionGuard.validateSession', 'validate(token)'],
        ['authorize', 'SessionGuard.auditSession', 'audit(token)']
      ]
    );
    assert.ok(calls.every((row) => row.end_line !== null));
    assert.ok(calls.every((row) => row.start_col !== null));
    assert.ok(calls.every((row) => row.end_col !== null));
    assert.ok(calls.every((row) => row.adapter_id === TS_JS_SEMANTIC_ADAPTER_ID));
    assert.ok(calls.every((row) => row.provenance.startsWith('method-alias-call:')));
  } finally {
    db.close();
  }
});
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        ...
        ['authorizeFromContext', 'SessionGuard.validateSession', 'aliasContext.guard.validateSession(token)'],
        ['authorizeFromContext', 'SessionGuard.auditSession', 'parameterAliasContext["backup"].auditSession(token)'],
        ...
        ['authorizeFromContext', 'SessionGuard.auditSession', 'aliasAuditGuard.auditSession(token)']
      ]
    );
    assert.deepEqual(calls.map((row) => row.start_line), [13, 13, 13, 13, 23, 24, 25, 26, 27, 28, 29, 30]);
    assert.ok(calls.every((row) => row.end_line !== null));
    assert.ok(calls.every((row) => row.start_col !== null));
    assert.ok(calls.every((row) => row.end_col !== null));
    assert.ok(calls.every((row) => row.adapter_id === TS_JS_SEMANTIC_ADAPTER_ID));
...
test('TypeScript JavaScript adapter records parser-backed awaited factory return type instance method dispatch spans', async () => {
  const repoRoot = await mkdtemp(path.join(tmpdir(), 'parallax-ts-awaited-factory-receiver-spans-'));
  await mkdir(path.join(repoRoot, 'src'), { recursive: true });
  ...
  await writeFile(path.join(repoRoot, 'src/guard.ts'), [
    ...
    '}',
    '',
    'async function createGuard(): Promise<SessionGuard> {',
    '  return new SessionGuard();',
    '}',
    '',
    'const createInferredGuard = async (): Promise<SessionGuard> => new SessionGuard();',
    '',
    'export async function authorize(token: string): Promise<boolean> {',
    '  return (await createGuard()).validateSession(token)',
    '    && (await createInferredGuard()).validateSession(token);',
    '}',
    ''
  ].join('\n'));
  ...
  try {
    ...
    assert.deepEqual(
      calls.map((row) => [row.source_symbol, row.target_symbol, row.snippet]),
      [
        ['authorize', 'SessionGuard.validateSession', '(await createGuard()).validateSession(token)'],
        ['authorize', 'SessionGuard.validateSession', '(await createInferredGuard()).validateSession(token)']
      ]
    );
    assert.deepEqual(calls.map((row) => row.start_line), [14, 15]);
    assert.ok(calls.every((row) => row.end_line !== null));
    assert.ok(calls.every((row) => row.start_col !== null));
    assert.ok(calls.every((row) => row.end_col !== null));
    assert.ok(calls.every((row) => row.adapter_id === TS_JS_SEMANTIC_ADAPTER_ID));
...
test('TypeScript JavaScript adapter records parser-backed optional-chaining receiver dispatch spans', async () => {
  const repoRoot = await mkdtemp(path.join(tmpdir(), 'parallax-ts-optional-chain-receiver-spans-'));
  await mkdir(path.join(repoRoot, 'src'), { recursive: true });
  ...
  await writeFile(path.join(repoRoot, 'src/guard.ts'), [
    ...
    '  const guard: SessionGuard = new SessionGuard();',
    '  return guard?.validateSession(token)',
    '    && createGuard()?.validateSession(token);',
    '}',
    ''
  ].join('\n'));
  ...
  try {
    ...
    assert.deepEqual(calls.map((row) => row.start_line), [13, 14]);
    assert.ok(calls.every((row) => row.end_line !== null));
    assert.ok(calls.every((row) => row.start_col !== null));
    assert.ok(calls.every((row) => row.end_col !== null));
    assert.ok(calls.every((row) => row.adapter_id === TS_JS_SEMANTIC_ADAPTER_ID));
...
test('TypeScript JavaScript adapter records parser-backed array element typed receiver dispatch spans', async () => {
  const repoRoot = await mkdtemp(path.join(tmpdir(), 'parallax-ts-array-element-receiver-spans-'));
  await mkdir(path.join(repoRoot, 'src'), { recursive: true });
  ...
  await writeFile(path.join(repoRoot, 'src/guard.ts'), [
    ...
    'class Coordinator {',
    '  private guards: GuardList;',
    '  private pair: GuardPair;',
    '  constructor(guards: GuardList, private readonlyPair: ReadonlyGuardPair) {',
    '    this.guards = guards;',
    '    this.pair = [guards[0], new AuditGuard()];',
    '  }',
    '  authorize(token: string): boolean {',
    '    return this.guards[0].validateSession(token)',
    '      && this.pair[0].validateSession(token)',
    '      && this.pair[1].auditSession(token)',
    '      && this.readonlyPair[0].validateSession(token)',
    '      && this.readonlyPair[1].auditSession(token);',
    '  }',
    '}',
    '',
  ...
  try {
    ...
    assert.deepEqual(
      calls.map((row) => [row.source_symbol, row.target_symbol, row.snippet]),
      [
        ['Coordinator.authorize', 'SessionGuard.validateSession', 'this.guards[0].validateSession(token)'],
        ['Coordinator.authorize', 'SessionGuard.validateSession', 'this.pair[0].validateSession(token)'],
        ['Coordinator.authorize', 'AuditGuard.auditSession', 'this.pair[1].auditSession(token)'],
        ['Coordinator.authorize', 'SessionGuard.validateSession', 'this.readonlyPair[0].validateSession(token)'],
        ['Coordinator.authorize', 'AuditGuard.auditSession', 'this.readonlyPair[1].auditSession(token)'],
