...
- docs/ADR 정리: Phase 6 foundation부터 Protobuf/AsyncAPI consumer resolver까지 decisions log D-037로 갱신
...
6. `feat(adapters): add Python Go Rust adapter v0`
   ...
   - no full package resolver; deterministic repo-local relation first
...
다음 제품 slice는 full parser/LSP depth와 build-system/package resolver로 넓히는 것이다.
