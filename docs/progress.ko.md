...
| Phase B | MCP retrieval depth + resource pagination | context/search/explain/telemetry/doctor/session import v0/v1와 search retrieval depth v0는 완료. 다음 slice는 persistent FTS projection, path/entity/relation diversification, returned bytes/token budget, report/graph pagination |
...
1. `impact_trace_search_context`에 path/entity/relation diversification과 returned bytes / estimated token budget을 적용한다.
2. relation_evidence/facts persistent FTS projection과 retrieval/context effectiveness bench를 추가해 Recall@budget, returned bytes, resource expansion rate를 측정한다.
