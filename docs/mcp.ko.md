...
| `parallax_query` | 인덱싱된 entity/relation 그래프에 읽기전용 Cypher 서브셋(정/역/가변 길이 hop, label, WHERE =/CONTAINS, 투영, LIMIT)을 실행하고 JSON 행과 질의한 index run, 탐색 가능한 entity 리소스를 함께 반환 | 예 |
