...
package resolver 已开始使用 npm lockfile，但大多数 lockfile ecosystem 和 semver impact 仍未完成。

- [ ] 基于 lockfile 的 transitive 依赖图 (npm·pip·poetry·go·cargo·maven·gradle)
  - 进展：npm `package-lock.json` v2/v3 的 transitive package entry 会被索引为带 locked version 与 evidence span 的 lockfile-derived `DEPENDS_ON` package relation。
