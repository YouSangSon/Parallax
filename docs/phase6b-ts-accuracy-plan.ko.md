...
> **상태:** `/autoplan` + `/team-builder` 이후 사용자 stack 정정 반영. 2026-05-11 현재 ImpactBench, adapter pack v0 routing, TS/JS parser-backed import span v0, JVM/Spring lightweight evidence span v0가 landed. 파일명은 기존 링크 유지를 위해 유지한다.
...
현재 얇은 spine은 `bench/impact-bench.ts`와 `npm run bench`로 시작한다. report는 기본적으로 `.impact-trace/bench/impact-bench-report.json`에 쓰며, `.impact-trace/`는 gitignore 대상이라 반복 실행해도 작업트리를 오염시키지 않는다. 2026-05-11 기준 fixture는 TS/JS alias/re-export/type-only/namespace/dynamic import/require span, Spring Boot `@ConfigurationProperties`, `application.properties`, JPA/Spring Data, `@DataJpaTest`, Feign/WebClient/RestTemplate까지 expected relation으로 검증한다. TS/JS parser-backed import span v0와 JVM/Spring lightweight evidence span v0 이후 `spanCompleteness >= 0.85`가 bench gate로 승격됐다. 이 spine은 Phase 6B 전체 완료 선언이 아니라, 이후 Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS adapter depth pass가 개선됐는지 숫자로 확인하기 위한 기준선이다.
...
- TS/JS import-backed evidence와 import 기반 `VERIFIES`, JVM/Spring endpoint/declaration/config/test evidence는 bounded snippet + line/col range를 저장하고 ImpactBench `spanCompleteness >= 0.85`를 유지
