...
| `parallax_contract_diff` | 현재 지원 contract 파일을 인덱싱된 workspace baseline과 비교해 compact한 breaking-change impact를 반환 | 아니오 |
...
| `parallax_resolve_cross_repo_contracts` | 동기화된 workspace DB view에서 cross-repo provider/consumer contract link를 미리보고 `cross_repo_links`는 변경하지 않음 | 예 |
...
| `parallax_query` | 인덱싱된 entity/relation 그래프에 읽기전용 Cypher 서브셋(정/역/가변 길이 hop, label, WHERE =/CONTAINS, 투영, COUNT 집계, ORDER BY, LIMIT)을 실행하고 JSON 행과 질의한 index run, 탐색 가능한 entity 리소스를 함께 반환 | 예 |
| `parallax_co_change` | git 코-체인지(CO_CHANGES)로 특정 파일과 이력상 결합된 파일을 결합 강도순으로 반환 — 정적 그래프가 놓치는 상관 결합을 heuristic 신뢰도로 표면화 | 예 |
...
matching되는 workspace breaking link가 이미 있으면 `parallax_analyze_diff`는 `parallax analyze`와 같은 `crossRepoImpacts` 섹션을 반환한다. 이 MCP tool은 workspace resolution 관점에서 read-only로 유지된다. 저장된 evidence를 표면화할 뿐 새 cross-repo link를 만들지 않는다.

read-only cross-repo consumers, providers, preview tool은 이미 동기화된 workspace DB view를 읽는다. `.parallax/workspace.json`을 다시 동기화하지 않고 `cross_repo_links`를 변경하지 않는다.

## MCP prompts

Prompt는 워크플로 템플릿이다(`prompts/list`로 노출되고 `prompts/get`으로 가져온다). 읽기전용 tool들을 일관된 조사 흐름으로 엮는 법을 에이전트에게 가르친다. 각 prompt는 analyze → context → query/co-change → remember 순서를 담은 user 메시지를 돌려주며, 고신뢰(proven > inferred > heuristic) 신호를 우선한다. 둘 다 선택적 `changedFiles` 문자열을 받는다.

| Prompt | 목적 |
| :--- | :--- |
| `impact_workflow` | 변경의 blast radius를 처음부터 끝까지 매핑: `parallax_analyze_diff` → `parallax_context_for_change` → `parallax_query` / `parallax_co_change` → `parallax_remember` |
| `triage_change` | 변경이 위험한지 빠르게 분류하고 무엇을 검증할지 결정한 뒤 판정을 기록 |
