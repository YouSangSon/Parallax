...
本文档只记录*当前可以无阻碍推进的方向*。重大决策都在不破坏 [invariants.md](invariants.md) 的前提下进行。
...
- [ ] 研究将 write surface 拆分为独立权限模型后引入（遵循 [invariants.md](invariants.md) I-8）
...
- [ ] 基于多语言 fixture 的 deterministic bench harness
- [ ] 在 embedding 模型 / LLM provider 交叉时对 recall 质量的回归 detection
- [ ] 在 CI 中每个 PR 自动报告 bench delta
