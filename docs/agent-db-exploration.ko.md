...
| Phase 2 임베딩 통합 | `@huggingface/transformers` ONNX in-process, default `Xenova/multilingual-e5-base`, env로 swap, async `rememberOnRepo` 래퍼 (embedding을 SQLite tx 바깥에서 계산) | (이번 commit) |
