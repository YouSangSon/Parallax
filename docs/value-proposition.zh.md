...
> - 架构地图: [`architecture.zh.md`](architecture.zh.md)
> - 验证 gate: [`verification.zh.md`](verification.zh.md)
> - 当前 roadmap: [`roadmap.zh.md`](roadmap.zh.md)

---

## 1. 一句话概括

> **一个本地工具，把"代码库地图"和"昨天想法的笔记"一起交到 AI 编码工具（Claude Code、Codex 等）手里。使用单个 SQLite 存储，不需要托管数据库或远程 API。**
...
### 进行中 / 未实现 🟡
- Parser-backed precision 仍然因语言而异。TypeScript/JavaScript 覆盖最深，JVM/Python/Go/Rust 和其他语言仍有 known gap。
- Cross-repo 与 contract intelligence 是 active v0 功能，不是完整的 enterprise dependency map。它只覆盖用户登记的 local repository 与 contract。
- Team-wide fact sync 仍是 future work。当前 local `.parallax/impact.db` 在 future tooling 明确移动 fact 前仍按 checkout 保留。
- IDE extension 与外部系统自动 sync 仍是 roadmap item。

当前验证数字和 dependency 数量变化很快，因此不在本文固定。以 `npm test`、`npm run test:dogfood`、`npm run bench` 和 `package.json` 作为 source of truth。
...
### B. 扩展到代码之外 —— 微服务时代的核心
- 深化 workspace catalog，超越显式登记的 local repository
