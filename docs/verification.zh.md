...
| `npm run docs:lint` | 对被跟踪的 Markdown 运行 `scripts/docs-lint.js` | 编辑任何文档后 |
...
`test:fixtures` 是 `npm test` 的别名，`test:benchmark` 是 `npm run bench` 的别名。
...
- `tests/impact-bench.test.ts` 作为 `npm test` 的一部分运行 bench，并断言报告形态、pin 住的 **76** 个期望 relation，以及 score/recall 阈值。
...
`scripts/docs-lint.js`（通过 `npm run docs:lint` 运行）是对每个被跟踪的 `*.md` 文件的 static gate。它强制：

- **无禁止内容**——本地机器路径、restore-point 元数据，以及类似密钥的字符串（私钥、GitHub token、`sk-...` key）。此扫描在包含 fenced code block 的 raw 文本上运行。
...
npm run build
npm test            # 快速 unit suite（不含 dogfood guard）
npm run test:dogfood
npm run bench
```

dogfood guard 与 bench 作为各自的 step 在 unit suite 之后运行，因此 unit test 漏掉的回归仍会让 CI 失败。
