...
업데이트: 2026-05-12 (Go workspace/replace resolver v0)
...
15. ✅ `package.json`, `pom.xml`, `build.gradle(.kts)`, Gradle `libs.versions.toml`, `go.mod`, `go.work`, `Cargo.toml`, `pyproject.toml` manifest-only build-system/package resolver v0와 Maven POM property interpolation v0, Go workspace/replace resolver v0를 완료했다.
