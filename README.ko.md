...
| **Contract diff** | OpenAPI, GraphQL, Protobuf, AsyncAPI surface diff를 `breaking` / `non-breaking` / `unknown`으로 분류하고, OpenAPI response format change와 enum-value removal도 감지 |
| **Consumer impact** | removed endpoint/operation, field removal/type/format/enum change, required request field 추가 등을 known consumer와 연결 |
