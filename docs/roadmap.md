...
> 마지막 업데이트: 2026-05-10 (session import v0 landed; agentmemory follow-up roadmap aligned around retrieval depth/resource pagination/supersession)
...
| active next | FTS5/BM25 projection for context search |
| active next | semantic recall lane for `impact_trace_search_context` (`semanticRank`) |
| active next | graph proximity lane over `relations` (`graphProximityRank`) |
