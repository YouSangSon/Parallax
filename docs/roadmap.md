...
> 마지막 업데이트: 2026-05-11 (UI Explorer v0 + TS/JS/JVM/Spring/Python/Go/Rust spans + OpenAPI contract impact baseline landed; next is workspace resolver)
...
다음 live work는 [Phase 6B Multi-language + Spring Boot Adapter Pack v0 plan](phase6b-ts-accuracy-plan.ko.md)이다. 완료된 항목: ImpactBench thin spine, TS/JS/JVM-Spring/Python/Go/Rust v0 adapter routing, Spring Boot endpoint/config/persistence/test/client fixture coverage, adapter attribution scoring, TS/JS parser-backed import span v0, JVM/Spring lightweight evidence span v0, Python/Go/Rust lightweight evidence span v0, OpenAPI contract baseline + implementer reverse-link v0. 아직 완료로 표시하지 않는 항목: workspace loader/resolver, cross-repo contract consumer mapping, full parser/LSP depth.
...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline은 landed. 다음은 workspace catalog와 cross-repo consumer/provider resolver. |
...
| active next | workspace resolver — OpenAPI baseline을 multi-repo consumer/provider graph와 contract diff로 확장 |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-025)
