...
> 마지막 업데이트: 2026-05-11 (UI Explorer v0 + TS/JS/JVM/Spring/Python/Go/Rust spans + OpenAPI contract impact baseline + workspace catalog v0 + cross-repo contract resolver v0 + OpenAPI endpoint/schema diff v0 + MCP workspace/contract resources v0 landed; next is nested schema and protobuf/GraphQL/AsyncAPI contract diff)
...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0, endpoint-surface contract diff v0, JSON/YAML flat schema/body diff v0는 landed. 다음은 nested schema와 protobuf/GraphQL/AsyncAPI 확장. |
...
| active next | nested schema/protobuf contract diff — OpenAPI allOf/oneOf/nested body rules와 protobuf/GraphQL/AsyncAPI contract impact로 확장 |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-029)
