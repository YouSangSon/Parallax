...
같은 `Entity`와 `Relation` 모델로 연결하는 것이다. 더 길게는 사업계획서, PRD, 회의록,
의사결정 기록, KPI, 고객/영업 문서 같은 회사 업무 산출물도 같은 모델에 올린다.
SQLite가 canonical store가 되고, graph DB는 필요할 때 파생하는 projection이다.
...
| Company-work aware impact | 코드뿐 아니라 사업계획서, PRD, 회의록, 정책, KPI, 고객/영업 문서도 영향 그래프의 대상이다. |
...
| MCP | 공식 MCP SDK 기반 stdio server, read-only `impact_trace_analyze_diff`와 report/entity/graph/coverage resources 제공 |
| Storage | repo-local SQLite, legacy `files/symbols/edges`, canonical `entities/relations/relation_evidence`, workspace/contract/work-artifact 확장 schema 병행 저장 |
| Indexing | TS/JS/Markdown + Python/Go/Rust/Java/Kotlin/C#/C/C++ + shell/YAML/JSON/TOML/Dockerfile/Makefile/Terraform/protobuf/GraphQL/CODEOWNERS 기본 휴리스틱 추출 |
| Report | canonical `relations` 기반 bounded multi-hop traversal, legacy `edges` fallback, `changed`, `affected`, `actions`, `evidence`, `warnings` report model |
| Visualization | report 범위의 Mermaid/JSON/DOT graph export 제공, web explorer는 예정 |
...
| analyzer 입력이 아직 file 중심 | symbol, package, workflow, resource 단위 분석은 schema와 모델은 준비됐지만 resolver가 더 필요하다. |
| regex 기반 TS/JS 추출 | re-export, path alias, type-only import, call/reference를 놓친다. |
| regex/mention 기반 multi-hop 분석 | side effect 전파는 가능하지만 semantic 정확도는 adapter 품질에 묶인다. |
| snapshot isolation 미완성 | indexing 중 completed index가 오염될 수 있다. |
| source span과 provenance 부족 | report가 왜 그런 결론을 냈는지 감사하기 어렵다. |
| resource limit이 파일 크기 중심 | 파일 수, 전체 시간, relation fan-out, memory budget을 더 정교하게 제한해야 한다. |
| cross-repo resolver 부재 | schema는 준비됐지만 프로젝트 A의 API/gRPC 변경을 프로젝트 B consumer까지 실제로 연결하는 resolver가 필요하다. |
| 회사 업무 artifact adapter 부재 | 사업계획서/PRD/회의록/고객 문서를 저장 모델에 올릴 수는 있지만 connector와 추출 규칙이 필요하다. |
...
| Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS adapter v0 + trusted evidence | 실제 stack의 첫 신뢰 레인으로 선언/import/test와 Spring Boot endpoint/config/persistence relation을 확보한다. |
...
| `work_artifacts` | 사업계획서, PRD, 회의록, KPI, 고객 문서 같은 회사 업무 산출물 metadata를 저장한다. |
| `artifact_links` | 업무 산출물과 code/entity 사이의 요구사항, 근거, 의사결정, 고객 영향 relation을 저장한다. |
...
| `business_plan` | 사업계획서, GTM plan, pricing plan |
| `requirement` | PRD 요구사항, acceptance criteria, 고객 요청 |
| `decision` | ADR, 회의 결정, 승인/거절 기록 |
| `meeting_note` | 회의록, 인터뷰 노트, 콜 노트 |
| `metric` | KPI, OKR, dashboard metric definition |
| `customer_artifact` | 고객 요구사항 문서, sales note, support ticket summary |
...
| `IMPLEMENTS` | code, route handler, RPC handler가 contract를 이행한다. |
...
| Multi-language + Spring Boot adapter pack v0 | P1 | TS/JS module/symbol, Java/Kotlin package/class/function-ish declaration, Spring endpoint/config/persistence/test/client, Python module/function/class, Go package/function/type, Rust module/item | declares, imports, re-exports, depends-on, verifies, configures, implements |
| Package/workspace adapter | P1 | package, script, workspace | depends-on, verifies, generates |
| Deep language adapters beyond v0 | P2 | TS/JS, Java/Kotlin/Spring, Python, Go, Rust semantic details beyond declaration/import/test/config | references, calls, resolved types, framework wiring |
...
| 작업 | 완료 기준 |
|---|---|
| Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS adapter v0 + trusted evidence | 각 언어의 선언/import/test relation, source span, git snapshot evidence를 추출한다. Spring Boot는 endpoint/config/persistence/test/client relation을 first-class로 다룬다. |
...
| 언어 adapter는 실제 stack v0를 먼저 닫고 이후 tier 방식으로 깊힌다. | Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS를 같은 모델에 올리고, Spring Boot endpoint/config/persistence/test/client relation을 first-class로 다룬다. | 한 언어의 full semantic analysis만 먼저 완성한다. |
| 다중 언어와 system model을 함께 잡는다. | 실제 repo는 Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS와 YAML/shell/infra/policy가 섞인다. | 단일 언어 adapter를 깊게 파는 순서 |
