...
| W4 | **Richer contract property signatures** — ✅ response enum-value removal, response format changes, response nullable additions, and request enum-value removal are now captured for OpenAPI via richer property signatures, compat schemaVersion 5, persisted breaking-change provenance, and `contractDiffQuality` bench cases. Remaining: request format narrowing/broadening semantics and response optionality rules. The substance of "nested-schema-level". | M | MED-HIGH |
...
**Sequencing remaining work:** W4 follow-ups (request format semantics and response optionality rules) then W5 JSON Schema reuse → W3 (biggest scope, monorepo users). W1/W2/W6 are already shipped.
