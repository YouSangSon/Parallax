...
> **작성:** 2026-05-03 (사전 design doc), 2026-05-04 branch 진행 상태 반영, 2026-05-09 main 반영 상태 정리, 2026-05-11 Phase 6B 진행 상태 반영, 2026-05-12 Protobuf/GraphQL/AsyncAPI contract diff와 GraphQL consumer resolver 반영
> **상태:** foundation subset은 `main`에 반영됨 (`3cba0a2`). Phase 6B에서는 multi-language/Spring/Python/Go/Rust/TS/JS spans, OpenAPI contract baseline, workspace catalog v0, cross-repo contract resolver v0, GraphQL consumer resolver v0, OpenAPI endpoint/nested schema diff v0, Protobuf contract diff v0, GraphQL contract diff v0, AsyncAPI contract diff v0, MCP workspace/contract resources v0가 landed. 다음 slice는 protobuf/AsyncAPI consumer resolver다.
> **참고:** [decisions.ko.md](decisions.ko.md) (D-001..D-036) · [impact-trace-plan.ko.md](impact-trace-plan.ko.md) (원래 P0/P1 ledger) · [roadmap.md](roadmap.md) (A1/A5 row) · [progress.ko.md](progress.ko.md).
...
- ✅ GraphQL consumer resolver v0 — operation document top-level selection과 provider root field link 저장
...
- Phase 7: protobuf/AsyncAPI consumer resolver
...
**최종 결정(2026-05-03): C with 2 refinements.** §6의 "Decided" 블록 참고. 이 branch는 해당 형태를 구현했고 이후 `decisions.ko.md`는 D-036까지 승격된 결정을 보유한다.
...
- [x] **6.A.1** — `docs/decisions.ko.md`에 D-019..D-036 결정 로그 추가
...
- [x] ADR D-019..D-036 정식 승격 (`decisions.ko.md`)
...
1. protobuf/AsyncAPI consumer resolver
