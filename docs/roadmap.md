...
> 마지막 업데이트: 2026-05-10 (Phase 6B ImpactBench + multi-language adapter pack v0 routing landed; agentmemory adoption boundary documented; MCP search context RRF ranking v1 landed)
...
다음 live work는 [Phase 6B Multi-language + Spring Boot Adapter Pack v0 plan](phase6b-ts-accuracy-plan.ko.md)이다. 완료된 항목: ImpactBench thin spine, TS/JS/JVM-Spring/Python/Go/Rust v0 adapter routing, Spring Boot endpoint/config/persistence/test/client fixture coverage, adapter attribution scoring. 아직 완료로 표시하지 않는 항목: parser-backed depth pass, persisted source span/range 확대, `index_runs` snapshot metadata(commit/dirty/branch), workspace loader/resolver.
...
| A2 | parser-backed source span + git snapshot depth pass | 다음 slice. source location 신뢰도와 stale-index 판단을 강화한다. |
...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | workspace catalog의 첫 사용 사례. |
...
| `impact_trace_search_context` — keyword/path/symbol/relation/evidence 검색으로 agent가 파일을 덤프하지 않고 관련 entity를 찾는 context discovery. v0 landed; v10부터 repo-local telemetry append 가능 |
| `impact_trace_search_context` ranking v1 — keyword/relation/evidence stream을 RRF로 fuse하고 `rankSignals`를 노출. initial landed; FTS/BM25 + semantic recall + relation proximity depth pass는 후속 |
| context access telemetry — context pack/resource fetch가 실제로 쓰인 정도를 기록해 context 절감 효과를 측정. v0 landed: `context_tool_runs`, `context_resource_accesses`, `impact_trace_context_telemetry` |
| doctor/health surface — schema/index/coverage/adapter/vector/telemetry 상태를 read-only JSON으로 반환. landed: `impact-trace doctor`, `impact_trace_doctor` |
| opt-in session import/crystal — landed: `impact-trace import-session --file <path> --format codex|claude`가 redacted `session_summary`와 `references_file=file:<rel>` facts를 만들고 provenance로 연결. MCP surface에는 노출하지 않음 |
| context pack ranking/dedupe — 반복 file/doc payload 없이 top impact paths와 resource links 우선 반환 |
| `impact-trace://reports/{id}` resource (큰 페이로드 pagination) |
| `impact-trace://entities/{id}` / `impact-trace://evidence/{id}` resource — evidence resource v0 landed, pagination은 후속 |
