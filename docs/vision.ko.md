...
[English](vision.md) · **한국어** · [中文](vision.zh.md)
...
큰 변경을 제안하기 전 [invariants.md](invariants.ko.md)를 읽는다. 핵심: local-first 단일 SQLite, content-addressable fact, ADD-only migration, redact-then-embed, fetch-only (no SDK), explicit triggers (no daemon), read-only agent surface first, actions are recommendations, evidence first.
...
이 항목들은 검토 후 거부 — context는 [invariants.md](invariants.ko.md). 재제안하려면 별도 논의가 필요하다.
