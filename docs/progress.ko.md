...
업데이트: 2026-05-11 (cross-repo contract resolver v0)
...
**Phase 6B multi-language + Spring Boot adapter pack v0 + trusted evidence + OpenAPI contract baseline + workspace catalog v0 + cross-repo contract resolver v0**와
...
| Phase 6/7 | workspace/cross-repo resolver | contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0, OpenAPI endpoint-surface contract diff v0, JSON/YAML flat schema/body diff v0, MCP workspace/contract resources v0는 준비됨. nested schema와 protobuf/GraphQL/AsyncAPI diff는 예정 |
...
13. OpenAPI JSON/YAML flat schema/body diff를 완료했고, nested schema와 protobuf/GraphQL/AsyncAPI breaking-change 분류로 확장한다.
