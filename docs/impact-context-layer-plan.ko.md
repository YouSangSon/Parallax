...
비범위: UI 본구현, full call graph, nested/protobuf/GraphQL/AsyncAPI contract diff.
...
| OpenAPI endpoint/schema contract diff | v0 landed: removed endpoint를 breaking, added endpoint를 non-breaking으로 분류하고, JSON/YAML flat request/response schema breaking change의 known consumer impact를 저장 |
...
| breaking-change classifier | nested schema/protobuf/GraphQL/AsyncAPI contract diff가 downstream risk를 생성 |
