...
- OpenAPI endpoint surface와 JSON/YAML request/response nested schema diff를 `breaking`/`non-breaking`/`unknown`으로 분류하고 known consumer impact를 `BREAKS_COMPATIBILITY_WITH` link로 저장
...
- Obsidian write sync
- graph DB projection
- protobuf/GraphQL/AsyncAPI까지 포함한 full contract diff
- web graph explorer
- CodeQL adapter
...
누적 결정 로그: [decisions.ko.md (D-001..D-032)](docs/decisions.ko.md).
...
- [Architecture decisions log (D-001..D-032)](docs/decisions.ko.md) — 누적 ADR 로그
...
5. parser-backed/lightweight adapter depth pass와 source-span evidence 확대: TS/JS import spans, JVM/Spring lightweight spans, Python/Go/Rust lightweight spans, OpenAPI contract baseline, workspace catalog v0, cross-repo contract resolver v0, OpenAPI nested schema contract diff v0 landed
...
8. protobuf/GraphQL/AsyncAPI breaking-change impact 분석 추가
9. web graph explorer와 더 큰 graph filtering 추가
10. source-span evidence와 parser-level provenance 추가
11. protobuf/GraphQL/AsyncAPI breaking-change impact 분석 추가
