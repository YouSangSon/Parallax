...
- docs/ADR 정리: Phase 6 foundation부터 UI work artifact impact surface까지 decisions log D-043으로 갱신
...
5. **UI work artifact impact surface v0:** selected report의 policy, decision, PRD, requirement, proposal impact가 `workArtifacts`와 Work Artifacts panel로 분리된다. 본문은 싣지 않고 entity resource URI로 expand-on-demand한다.
