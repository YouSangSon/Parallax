...
| **Contract diff** | Classifies OpenAPI, GraphQL, Protobuf, and AsyncAPI surface diffs as `breaking` / `non-breaking` / `unknown`, including OpenAPI response nullable additions, response format/enum changes, and request enum-value removals |
| **Consumer impact** | Links removed endpoints/operations, field removal/type/nullable/format/enum changes, added required request fields, and request enum-value removals to known consumers |
