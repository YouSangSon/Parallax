...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0, GraphQL/Protobuf/AsyncAPI consumer resolver v0, generated-client/event topology v0, endpoint-surface contract diff v0, JSON/YAML nested schema/body diff v0, Protobuf contract diff v0, GraphQL contract diff v0, AsyncAPI contract diff v0는 landed. 다음은 richer topology와 full parser depth. |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-039)
