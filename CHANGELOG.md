...
- **UI work artifact impact surface v0** — `impact-trace ui` now separates policy, decision, PRD, requirement, and proposal impacts from the selected report into a compact Work Artifacts panel and bootstrap field without embedding document bodies.
- **UI work artifact metadata preview v0** — Work Artifacts rows now extract compact title/owner/status/updated metadata from Markdown frontmatter or a document-leading H1, while artifact body evidence remains omitted from the bootstrap payload.
- **UI work artifact freshness badge v0** — Work Artifacts rows now classify compact `updatedAt` metadata as stale/current/unknown against deterministic report time, so outdated policies, PRDs, requirements, decisions, and proposals surface without loading document bodies.
- **MCP work artifact context preview v0** — `impact_trace_context_for_change` now includes body-free `workArtifacts` entries with metadata, freshness, relation path, and entity resource URI while omitting work artifact evidence snippets from the compact context pack.
