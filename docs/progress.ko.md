...
업데이트: 2026-05-10 (Phase 6B adapter pack v0 + session import v0 + agentmemory follow-up planning)
...
| Phase B | MCP retrieval depth + resource pagination | context/search/explain/telemetry/doctor/session import v0/v1는 완료. 다음 slice는 FTS5/BM25, semanticRank, graphProximityRank, path/entity/relation diversification, returned bytes/token budget, report/graph pagination |
| Phase B | agentmemory-informed context lifecycle | [agentmemory 적용성 분석](agentmemory-adoption-review.ko.md)에 따라 platform 도입은 거부하고 pattern만 적용. RRF ranking initial slice, context telemetry v0, session import v0는 완료했고 explicit supersession과 bounded session facets는 후속 |
...
| 날짜 | 명령 | 결과 |
|---|---|---|
| 2026-05-10 | `npm test` | **191개 테스트 통과** |
...
1. `impact_trace_search_context`에 FTS5/BM25 projection, semanticRank, graphProximityRank를 추가한다.
2. context pack에 path/entity/relation diversification과 returned bytes / estimated token budget을 적용한다.
3. `impact-trace://reports/{id}/graph/{format}` pagination과 typed error envelope를 구현한다.
4. explicit supersession으로 오래된 decision/summary/policy fact를 명시적으로 대체한다.
5. retrieval/context effectiveness bench를 추가해 Recall@budget, returned bytes, resource expansion rate를 측정한다.
6. 같은 resource contract를 읽는 `impact-trace ui` workbench v0를 만든다.
