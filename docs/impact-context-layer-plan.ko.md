...
| [GraphQL Inspector](https://github.com/graphql-hive/graphql-inspector) / [diff docs](https://the-guild.dev/graphql/inspector/docs/commands/diff) | GraphQL schema diff에서 breaking/dangerous/non-breaking 분류와 change code taxonomy를 제공한다. | GraphQL v0는 removed root field, required arg/input 추가, output/input type 변경 rule을 compact signature로 가져왔다. consumer resolver v0는 Inspector의 document/operation usage 모델을 참고해 operation document top-level selection을 provider root field와 연결한다. enum/interface/union/directive/deprecation rule과 full validation은 후속 full parser pass에서 다룬다. | Hive/cloud registry, GitHub app, hosted checks를 core로 가져오지 않는다. |
...
이번 slice에서는 위 조사에서 공통으로 보이는 `search/read only/context budget/resource link` 패턴을 작게 가져와 `impact_trace_search_context` v0로 구현했고, 이어서 `agentmemory` 분석에서 확인한 RRF hybrid ranking을 initial v1로 반영했다. context access telemetry와 opt-in session import v0도 Impact-trace의 SQLite/provenance 경계 안에서 구현됐다. retrieval depth v0는 natural-language query용 FTS5/BM25 entity lane, 기존 `fact_embeddings` 기반 `semanticRank`, matched seed의 1-hop `graphProximityRank`까지 추가했다. budget/diversification v0는 optional `brief`/`standard`/`deep` budget으로 returned bytes / estimated tokens / omitted counts를 노출하고 `k>=3` 결과를 path prefix/entity kind/relation kind bucket으로 interleave한다. persistent retrieval v0는 schema v11 `relation_evidence`/selected `facts` FTS projection과 ImpactBench schema v2 Recall@budget/ablation report를 추가했다. entity persistent retrieval v0는 schema v14 `search_entities_fts`로 read-only temp FTS rebuild를 제거하고 semantic lane을 sqlite-vec ANN-first/brute-force fallback으로 바꿨다. resource contract v0는 JSON graph pagination과 typed MCP error envelope를 추가했다. explicit supersession v0는 `supersedesFactIds`, `fact_provenance.kind='supersedes'`, edge `tx_id`로 오래된 decision/summary/policy fact를 현재/as-of recall/profile에서 제외한다. context pack reuse v0는 schema v15 `context_packs`와 `impact-trace://context-packs/{contextPackId}` resource로 같은 context 배열을 반복 전송하지 않게 한다. UI Explorer v0는 `impact-trace ui`로 같은 data/resource shape를 read-only local workbench에서 사람이 검토하게 한다. UI workspace topology surface v0는 같은 workbench와 `/api/workspaces/{name}`에서 contract baseline, cross-repo link, event topology hint, resource URI를 compact preview로 보여준다. UI work artifact impact surface v0는 selected report의 policy/decision/PRD/proposal impact를 Work Artifacts panel로 분리하고 문서 본문 대신 entity resource URI를 남긴다. UI work artifact metadata/freshness preview v0는 Markdown frontmatter/문서 선두 H1에서 title/owner/status/updatedAt만 추출하고 report 생성 시각 기준 stale/current/unknown을 본문 없이 표시한다. MCP work artifact context preview v0는 같은 compact artifact metadata/freshness를 `context_for_change.workArtifacts`로 제공하고 artifact evidence snippet은 placeholder로 생략한다. TS/JS parser-backed import span v0, JVM/Spring lightweight evidence span v0, Python/Go/Rust lightweight evidence span v0는 같은 evidence shape에 bounded snippet과 line/col range를 채워 MCP/UI가 근거 위치를 바로 펼칠 수 있게 한다. workspace catalog v0는 `.impact-trace/workspace.json` local allowlist와 `workspace init/add-repo/list` CLI로 cross-repo resolver의 읽기 경계를 고정한다. cross-repo contract resolver v0는 indexed workspace repo의 OpenAPI endpoint와 HTTP consumer file literal, GraphQL provider root field와 consumer operation document, Protobuf generated-client/full-route call-site, AsyncAPI producer/consumer event call-site를 `cross_repo_links`로 저장해 contract diff의 downstream 후보를 작게 만든다. OpenAPI contract diff v0는 latest indexed OpenAPI endpoint surface와 current contract file을 비교해 removed endpoint를 breaking으로 분류하고 known consumer만 `BREAKS_COMPATIBILITY_WITH` link로 저장한다. JSON/YAML OpenAPI는 `contract_versions.compatibility_json`에 저장된 nested request/response schema path signature로 required property/status/type breaking change까지 작은 payload로 알려준다. Protobuf v0는 Buf의 taxonomy를 참고한 `protobuf-compat-v0` service/RPC/message field signature로 removed RPC와 response field removal/type change를 작은 payload로 알려준다. GraphQL v0는 `graphql-compat-v0` root/object/input signature로 removed root field와 schema field breaking change를 작은 payload로 알려준다. AsyncAPI v0는 `asyncapi-compat-v0` operation/channel/message payload signature로 removed operation과 message payload breaking change를 작은 payload로 알려준다. Build-system/package resolver v0는 manifest-only package graph를 만들어 package manifest 변경 시 dependent package manifests를 작은 payload로 알려준다.
...
| source span persistence | landed: report/MCP evidence가 line/col range를 노출하고 TS/JS import-backed relation, JVM/Spring endpoint/declaration/config/test relation, Python/Go/Rust declaration/test relation은 bounded span을 기록 |
...
비범위: UI 본구현, full call graph, cross-file generated-client usage graph, richer event topology inference, GraphQL/protobuf/AsyncAPI full parser depth.
...
| persisted context pack id / repeated-query reuse | v0 landed: schema v15 `context_packs`, `impact-trace://context-packs/{contextPackId}` resource, first full compact pack + repeated `context_pack_reference` response |
| `impact-trace ui` workbench | v0 landed: localhost read-only UI, DoctorReport/ImpactReport/GraphExport/Coverage/ContextPack bootstrap, list-first changed/affected/evidence/action panels |
...
| workspace topology view | v0 landed: contract baseline, provider/consumer link, event topology hint, `/api/workspaces/{name}` JSON |
| work artifact impact view | v0 landed: selected report와 context pack의 policy/decision/PRD/proposal impact를 별도 panel/lane과 entity resource URI로 표시. frontmatter/heading metadata와 stale/current/unknown freshness preview 포함, artifact evidence snippet은 bootstrap/context pack에서 생략 |
...
| AsyncAPI consumer resolver | v0 landed: provider event operation과 consumer/producer source/config event address call-site를 `cross_repo_links`로 연결하고 topology hint를 provenance에 저장 |
...
| UI owner/repo filter | v0 started: `impact-trace ui`가 workspace contract/link/topology preview를 제공. 다음은 owner/repo/service filter와 graph drill-down |
...
8. `feat(ui): add local report workbench` — landed v0
   - `impact-trace ui`
   - report selector, changed/affected/evidence/action panels, focused graph, doctor findings, coverage gaps, context pack summary

9. `test(ui): add workbench smoke for local UI`
   - nonblank HTML/API bootstrap
   - localhost server lifecycle and typed empty states
   - viewport overflow check remains a manual/browser smoke gate until Playwright is added as an explicit dev dependency
...
| Fixture | 검증 |
|---|---|
| TS/JS advanced imports | re-export, path alias, type-only import, namespace import, dynamic import, require, parser-backed evidence span |
