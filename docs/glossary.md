...
### fact_provenance
fact 사이의 evidence chain. `(fact_id, source_fact_id, kind)` — `kind`는 `'evidence'` (인덱서가 만든 코드 관찰) 또는 `'summary'` (Phase 3 reflective consolidation의 source). recall이 evidence chain을 walk하여 audit.
