...
> **상태:** foundation subset은 `main`에 반영됨 (`3cba0a2`). Phase 6B에서는 multi-language/Spring/Python/Go/Rust/TS/JS spans, OpenAPI contract baseline, workspace catalog v0, cross-repo contract resolver v0가 landed. 다음 slice는 contract diff다.
> **참고:** [decisions.ko.md](decisions.ko.md) (D-001..D-027) · [impact-trace-plan.ko.md](impact-trace-plan.ko.md) (원래 P0/P1 ledger) · [roadmap.md](roadmap.md) (A1/A5 row) · [progress.ko.md](progress.ko.md).
...
- Phase 7: breaking-change classifier · MCP `impact-trace://workspace|contract` resources
...
**최종 결정(2026-05-03): C with 2 refinements.** §6의 "Decided" 블록 참고. 이 branch는 해당 형태를 구현했고 이후 `decisions.ko.md`는 D-027까지 승격된 결정을 보유한다.
...
- [x] **6.A.1** — `docs/decisions.ko.md`에 D-019..D-027 결정 로그 추가
...
- [x] ADR D-019..D-027 정식 승격 (`decisions.ko.md`)
...
1. contract diff/breaking-change classification
2. MCP workspace/contract resources
