...
- [x] 다언어 fixture 기반 deterministic bench harness
  ...
  - 현재 contract-diff gate: bench에는 removed response required property, added request required property, response property type change, response format change, response enum-value removal에 대한 paired OpenAPI v1/v2 quality case가 포함되며, CI summary가 delta를 추적할 수 있도록 `contractDiffQuality`로 보고된다.
