...
6. agent가 더 필요한 경우에만 `impact://evidence/{id}` 또는 `impact://entity/{id}`를 fetch한다.
...
5. 큰 graph와 evidence는 `impact://report/{id}`, `impact://evidence/{id}`, `impact://graph/{id}`로 넘긴다.
...
flowchart TB
  ...
  ResourceLinks --> Report["impact://report/{id}"]
  ResourceLinks --> Entity["impact://entity/{id}"]
  ResourceLinks --> Evidence["impact://evidence/{id}"]
  ResourceLinks --> Graph["impact://graph/{id}"]
...
| `impact_trace_explain_entity` | entity 하나의 relation/evidence를 설명 | entity summary, incoming/outgoing relations, resource links |
...
| Resource URI | 내용 |
|---|---|
| `impact://report/{report_id}` | versioned report JSON |
| `impact://entity/{entity_id}` | entity profile, versions, static/dynamic facts |
| `impact://evidence/{evidence_id}` | redacted evidence detail |
| `impact://graph/{report_id}` | paginated graph nodes/edges |
| `impact://policy/{entity_id}` | policy/rule impact detail |
| `impact://workspace/{workspace_id}` | workspace repos/contracts/services |
| `impact://coverage/latest` | last index coverage and adapter gaps |
...
| `impact_trace_explain_entity` | entity 주변 relation/evidence resource 제공 |
| `impact://evidence/{id}` | source span과 redacted snippet fetch 가능 |
| `impact://graph/{id}` pagination | 큰 graph를 tool payload에 넣지 않음 |
...
3. `impact://evidence/{id}`와 `impact://graph/{id}`로 큰 payload를 필요할 때만 읽게 만든다.
