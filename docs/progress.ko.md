...
| Phase 6B | source span persistence | `relation_evidence` line/col/range 저장과 analyzer evidence output은 구현됨. TS/JS parser-backed import span v0로 bench `spanCompleteness`는 0.7692까지 개선됐고, 다음은 JVM/Spring 또는 workspace/contract resolver depth |
...
8. parser-backed depth pass를 JVM/Spring Boot/Python/Go/Rust까지 넓혀 relation span/confidence를 더 정밀화한다.
9. 실제 workspace catalog 파일을 정의하고 여러 repo를 등록/조회하는 CLI를 추가한다.
10. OpenAPI/protobuf/GraphQL/AsyncAPI contract baseline과 breaking-change 분류를 구현한다.
