...
Parallax는 불변 원칙 **I-8**([invariants.ko.md](invariants.ko.md) 참고)을 따른다 — agent surface는 안전한 read-only 분석 계층을 먼저 안정화하고, 쓰기 권한은 별도 모델과 리뷰 뒤에만 추가한다. 각 tool은 MCP `readOnlyHint` 어노테이션을 선언한다. 표에서 `readOnlyHint: true`로 표시된 tool은 순수 읽기다. `readOnlyHint: false`로 표시된 tool에는 응답하는 부수 효과로 context-pack과 telemetry 행을 저장하는 분석 tool, 그리고 명시적 메모리 쓰기와 branch 관리 tool이 포함된다. 이들 중 어느 것도 소스 트리를 수정하지 않는다 — action은 추천일 뿐이다(불변 원칙 **I-9**).
...
JSON graph resource는 `?limit=100&cursor=nodeOffset:edgeOffset`로 페이지를 나눌 수 있다. paged request에서 `limit` 기본값은 `100`이며 `1`부터 `500` 사이여야 한다. 다음 페이지 cursor는 `page.nextCursor`로 반환된다. 잘못된 pagination은 MCP `invalid_pagination`을 반환하고, UI는 같은 validation을 `invalid_request`로 매핑한다.
