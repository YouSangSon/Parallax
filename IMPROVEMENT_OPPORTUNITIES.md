...
| D7 | **SARIF / GitHub Code Scanning export** — ✅ **affected-file SARIF projection shipped** from `ImpactReport` via `parallax analyze --sarif-output <path> [--sarif-category <category>]`, plus evidence locations, relation paths, confidence rules, stable fingerprints, and docs for Code Scanning upload. ✅ **recommended verification actions** now emit as SARIF note results. Still open: contract breaks, adapter `knownGaps`, and coverage gaps. | M | HIGH |
