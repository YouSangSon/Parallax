...
已持久化的 report 和 report-scoped graph export 都从存储的 report JSON snapshot 中读取。后续 index run、carry-forward、retention、repair 或 canonical graph row 变更，都不能改变已有 report 表达的内容。只有当 persisted report 缺少 relation-bearing evidence 时，canonical graph row 才可以补充 legacy report。
