...
> 마지막 업데이트: 2026-05-11 (UI Explorer v0 + TS/JS/JVM/Spring/Python/Go/Rust spans + OpenAPI contract impact baseline + workspace catalog v0 + cross-repo contract resolver v0 landed; next is contract diff)
...
| A6 | OpenAPI / protobuf / GraphQL / AsyncAPI 어댑터 + cross-repo resolver | OpenAPI contract baseline, workspace catalog v0, OpenAPI provider/consumer resolver v0는 landed. 다음은 contract diff와 breaking-change classification. |
...
| active next | contract diff — OpenAPI baseline, workspace catalog, cross-repo links를 breaking/non-breaking change impact로 확장 |
...
- **왜 이 결정인지 알고 싶다** → [decisions.ko.md](decisions.ko.md) (D-001..D-027)
