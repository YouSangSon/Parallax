...
| **Contract diff** | Classifies OpenAPI, GraphQL, Protobuf, and AsyncAPI surface diffs as `breaking` / `non-breaking` / `unknown`, including OpenAPI response nullable additions, format changes, and enum-value removals |
| **Consumer impact** | Links removed endpoints/operations, field removal/type/nullable/format/enum changes, and added required request fields to known consumers |
