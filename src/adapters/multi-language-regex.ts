...
function collectTypeScriptJavaScriptObjectPropertyTypes(sourceFile: ts.SourceFile): TsObjectPropertyTypes {
  ...
  const addMembers = (typeName: string, members: ts.NodeArray<ts.TypeElement>): void => {
    const properties = propertyTypes.get(typeName) ?? new Map<string, ts.TypeNode>();
    for (const member of members) {
      if (!ts.isPropertySignature(member) || !member.type) continue;
      const propertyName = propertyNameText(member.name);
      if (propertyName) properties.set(propertyName, member.type);
    }
    if (properties.size > 0) propertyTypes.set(typeName, properties);
  };

  const visit = (node: ts.Node): void => {
    if (ts.isInterfaceDeclaration(node)) {
      addMembers(node.name.text, node.members);
...
function collectTypeScriptJavaScriptLocalInstanceBindings(
  ...
  const addObjectBindingPatternBindings = (
    ...
    for (const element of pattern.elements) {
      ...
      const propertyType = propertyName
        ? objectTypePropertyTypeNode(typeNode, propertyName, objectPropertyTypes, typeReferenceAliases)
        : undefined;
      ...
      ) ?? objectPropertyBindingFromInitializer(initializer, bindings, scopeName, propertyName);
      if (binding) addBinding(scopeName, element.name.text, binding);
    }
  };

  const addObjectPropertyBindings = (
    scopeName: string,
    localName: string,
    typeNode: ts.TypeNode | undefined,
    typeParameterConstraints: ReadonlyMap<string, TsLocalInstanceBinding>
  ): void => {
    for (const [propertyName, propertyType] of objectTypePropertyTypeEntries(
      typeNode,
      objectPropertyTypes,
      typeReferenceAliases
    )) {
      ...
      if (binding) addBinding(scopeName, `${localName}.${propertyName}`, binding);
    }
  };

  const addObjectPropertyBindingsFromInitializer = (
    scopeName: string,
    localName: string,
    initializer: ts.Expression | undefined
  ): void => {
    if (!initializer) return;
    const baseName = expressionIdentifierName(initializer);
    if (!baseName) return;
    const sourcePrefix = scopedLocalName(scopeName, `${baseName}.`);
    for (const [key, binding] of bindings.entries()) {
      if (!key.startsWith(sourcePrefix)) continue;
      const propertyName = key.slice(sourcePrefix.length);
      if (propertyName) addBinding(scopeName, `${localName}.${propertyName}`, binding);
    }
  };

  const addIndexedCollectionBindingsFromInitializer = (
    scopeName: string,
    localName: string,
    initializer: ts.Expression | undefined
  ): void => {
    ...
    const baseName = expressionIdentifierName(initializer);
    if (!baseName) return;
    const sourceBase = scopedLocalName(scopeName, baseName);
    for (const [key, binding] of bindings.entries()) {
      if (key === `${sourceBase}[]`) {
        addBinding(scopeName, `${localName}[]`, binding);
      } else if (key.startsWith(`${sourceBase}[`)) {
        addBinding(scopeName, `${localName}${key.slice(sourceBase.length)}`, binding);
      }
    }
  };

  const addArrayBindingPatternBindings = (
    ...
    const tupleElementTypes = tupleElementTypeNodes(typeNode, indexedCollectionAliases);
    const arrayElementType = tupleElementTypes.length === 0
      ? arrayElementTypeNode(typeNode, indexedCollectionAliases)
      : undefined;
    for (const [index, element] of pattern.elements.entries()) {
      ...
      const binding = typeBindingFromTypeNode(
        tupleElementTypes[index] ?? arrayElementType,
        typeReferenceAliases,
        typeUnionAliases,
        typeParameterConstraints
      ) ?? indexedCollectionElementBindingFromInitializer(initializer, bindings, scopeName, index);
      if (binding) addBinding(scopeName, element.name.text, binding);
    }
  };
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name)) {
      ...
      if (scope) {
        addObjectPropertyBindings(scope.name, node.name.text, node.type, typeParameterConstraints);
        addObjectPropertyBindingsFromInitializer(scope.name, node.name.text, node.initializer);
        addIndexedCollectionBindingsFromInitializer(scope.name, node.name.text, node.initializer);
        ...
        for (const [index, typeNode] of tupleElementTypeNodes(node.type, indexedCollectionAliases).entries()) {
          ...
          if (tupleElementBinding) addBinding(scope.name, `${node.name.text}[${index}]`, tupleElementBinding);
        }
      }
      ...
      const scope = enclosingLocalCaller(node, localCallables);
      if (scope) {
        addArrayBindingPatternBindings(
          node.name,
          node.type,
          node.initializer,
          scope.name,
      ...
      if (binding && scope) addBinding(scope.name, node.name.text, binding);
      if (scope) {
        addObjectPropertyBindings(scope.name, node.name.text, node.type, parameterConstraints);
        const elementBinding = typeBindingFromTypeNode(
          arrayElementTypeNode(node.type, indexedCollectionAliases),
          typeReferenceAliases,
          typeUnionAliases,
...
function indexedCollectionElementBindingFromInitializer(
  initializer: ts.Expression | undefined,
  bindings: ReadonlyMap<string, TsLocalInstanceBinding>,
  scopeName: string,
  index: number
  ...
  return bindings.get(scopedLocalName(scopeName, `${baseName}[${index}]`))
    ?? bindings.get(scopedLocalName(scopeName, `${baseName}[]`));
}

function objectPropertyBindingFromInitializer(
  initializer: ts.Expression | undefined,
  bindings: ReadonlyMap<string, TsLocalInstanceBinding>,
  scopeName: string,
  propertyName: string | undefined
): TsLocalInstanceBinding | undefined {
  if (!initializer || !propertyName) return undefined;
  const baseName = expressionIdentifierName(initializer);
  if (!baseName) return undefined;
  return bindings.get(scopedLocalName(scopeName, `${baseName}.${propertyName}`));
}
...
function objectTypePropertyTypeEntries(
  typeNode: ts.TypeNode | undefined,
  objectPropertyTypes: Readonly<TsObjectPropertyTypes>,
  typeReferenceAliases: ReadonlyMap<string, string>,
  seen = new Set<string>()
): Array<[string, ts.TypeNode]> {
  if (!typeNode) return [];
  if (ts.isTypeLiteralNode(typeNode)) {
    return typeNode.members.flatMap((member): Array<[string, ts.TypeNode]> => {
      if (!ts.isPropertySignature(member) || !member.type) return [];
      const propertyName = propertyNameText(member.name);
      return propertyName ? [[propertyName, member.type]] : [];
    });
  }
  if (ts.isTypeReferenceNode(typeNode)) {
    const typeName = classNameFromTypeReference(typeNode, typeReferenceAliases);
    if (!typeName || seen.has(typeName)) return [];
    seen.add(typeName);
    return [...(objectPropertyTypes.get(typeName)?.entries() ?? [])];
  }
  if (ts.isIntersectionTypeNode(typeNode)) {
    return typeNode.types.flatMap((childType) => objectTypePropertyTypeEntries(
      childType,
      objectPropertyTypes,
      typeReferenceAliases,
      seen
...
function objectTypePropertyTypeNode(
  typeNode: ts.TypeNode | undefined,
  propertyName: string,
  objectPropertyTypes: Readonly<TsObjectPropertyTypes>,
  typeReferenceAliases: ReadonlyMap<string, string>,
  ...
  if (ts.isTypeReferenceNode(typeNode)) {
    ...
    seen.add(typeName);
    return objectPropertyTypes.get(typeName)?.get(propertyName);
  }
  if (ts.isIntersectionTypeNode(typeNode)) {
    for (const childType of typeNode.types) {
      const propertyType = objectTypePropertyTypeNode(
        childType,
        propertyName,
        objectPropertyTypes,
        typeReferenceAliases,
...
function collectTypeScriptJavaScriptClassInstanceBindings(
  ...
  const addIndexedCollectionBindings = (
    ownerClassName: string | undefined,
    propertyName: string | undefined,
    typeNode: ts.TypeNode | undefined,
    typeParameterConstraints: ReadonlyMap<string, TsLocalInstanceBinding>
  ): void => {
    if (!ownerClassName || !propertyName) return;
    const elementBinding = typeBindingFromTypeNode(
      arrayElementTypeNode(typeNode, indexedCollectionAliases),
      typeReferenceAliases,
      typeUnionAliases,
    ...
    if (elementBinding) addBinding(ownerClassName, `${propertyName}[]`, elementBinding);
    for (const [index, elementType] of tupleElementTypeNodes(typeNode, indexedCollectionAliases).entries()) {
      const tupleElementBinding = typeBindingFromTypeNode(
        elementType,
        typeReferenceAliases,
        typeUnionAliases,
      ...
      if (tupleElementBinding) addBinding(ownerClassName, `${propertyName}[${index}]`, tupleElementBinding);
    }
  };
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isPropertyDeclaration(node)) {
      ...
      addIndexedCollectionBindings(ownerClassName, propertyName, node.type, typeParameterConstraints);
      ...
      addIndexedCollectionBindings(ownerClassName, node.name.text, node.type, typeParameterConstraints);
...
function tupleElementTypeNodes(
  ...
  for (const element of node.elements) {
    const elementType = tupleElementTypeNode(element);
...
function tupleElementTypeNode(node: ts.TypeNode | ts.NamedTupleMember): ts.TypeNode | undefined {
  if (ts.isNamedTupleMember(node)) return tupleElementTypeNode(node.type);
...
function arrayElementReceiverBinding(
  ...
  if (!baseName) return undefined;
  const tupleIndex = numericElementAccessIndex(unwrapped.argumentExpression);
  if (tupleIndex !== undefined) {
    const tupleBinding = localInstanceBindings.get(scopedLocalName(sourceScopeName, `${baseName}[${tupleIndex}]`));
    if (tupleBinding) return tupleBinding;
  }
  return localInstanceBindings.get(scopedLocalName(sourceScopeName, `${baseName}[]`));
}

function objectPropertyReceiverBinding(
  expression: ts.Expression,
  localInstanceBindings: ReadonlyMap<string, TsLocalInstanceBinding>,
  sourceScopeName: string
): TsLocalInstanceBinding | undefined {
  const propertyPath = expressionLocalObjectPropertyName(expression);
  return propertyPath
    ? localInstanceBindings.get(scopedLocalName(sourceScopeName, propertyPath))
    : undefined;
}

function classElementReceiverBinding(
  expression: ts.Expression,
  ...
  if (!ts.isElementAccessExpression(unwrapped)) return undefined;
  if (elementAccessMemberName(unwrapped)) return undefined;
  const propertyName = thisPropertyName(unwrapped.expression);
  if (!propertyName) return undefined;
  const tupleIndex = numericElementAccessIndex(unwrapped.argumentExpression);
  if (tupleIndex !== undefined) {
    const tupleBinding = classInstanceBindings.get(scopedLocalName(ownerClassName, `${propertyName}[${tupleIndex}]`));
    if (tupleBinding) return tupleBinding;
  }
  return classInstanceBindings.get(scopedLocalName(ownerClassName, `${propertyName}[]`));
}

function numericElementAccessIndex(node: ts.Expression | undefined): number | undefined {
  if (!node) return undefined;
  const expression = unwrapTypeScriptJavaScriptReceiverExpression(node);
...
function expressionLocalObjectPropertyName(node: ts.Expression): string | undefined {
  const access = memberAccessExpression(node);
  const baseName = access ? expressionIdentifierName(access.expression) : undefined;
  const memberName = access ? memberNameFromAccessExpression(access) : undefined;
  return baseName && memberName ? `${baseName}.${memberName}` : undefined;
}
...
function localCallTargetsForExpression(
...

  {
    ...
    const instance = receiverTypeBindingFromExpression(
      ...
      : undefined)
    ?? objectPropertyReceiverBinding(access.expression, localInstanceBindings, sourceScopeName)
  ...
  }
  {
    ...
    const instance = className
      ? propertyName
        ...
        : classElementReceiverBinding(access.expression, classInstanceBindings, className)
      : undefined;
    if (instance) {
      return localTypeMemberCallablesForBinding(
        localCallables,
