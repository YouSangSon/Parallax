...
| 2026-05-13 | Cargo workspace/path resolver v0 | build-system/package resolver가 `[workspace].members`를 member `Cargo.toml`로 연결하고, Cargo local `path` dependency와 `workspace = true` dependency metadata를 package graph에 반영한다. Cargo 실행, feature resolver, target cfg evaluation, lockfile/transitive graph는 후속이다. |
