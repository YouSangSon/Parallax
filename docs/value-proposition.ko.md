...
`.impact-trace/impact.db` 한 파일에 모든 것이 들어간다.
...
요약: Claude·GPT 메모리는 *"나(에이전트)와 너(사용자)의 관계 메모"*. Impact Trace는 *"내가 만지는 코드베이스의 작동 기억"* — 보완 관계.

### vs. MCP memory server (커뮤니티 표준 메모리)

대부분의 MCP memory 서버는 단순 key-value 또는 텍스트 메모리.
Impact Trace는 **MCP 위에 시간/분기/인과/코드그래프를 1급으로 얹은** 메모리.

### vs. Sourcegraph / CodeQL

| 축 | Sourcegraph·CodeQL | Impact Trace |
|---|---|---|
...
기존 도구가 *코드를 검색하는 사람*을 위한 것이라면, Impact Trace는 *코드를 만지는 AI*를 위한 것.

### vs. 그래프 DB(Neo4j 등) + 벡터 DB 조합

직접 만들면: 그래프 DB 서버 + 벡터 DB + ETL 파이프라인 + 인증/권한.
Impact Trace는 SQLite 한 파일 + sqlite-vec 확장으로 같은 본질을 표현. 운영 복잡도 1/10.
...
```bash
# 팀원 A
impact-trace remember --entity file:src/auth.ts --attribute decision --value '"X 방식"'
impact-trace export --since last-sync > facts/2026-04-30-A.jsonl
git add facts/ && git commit -m "share auth decisions" && git push

# 팀원 B
git pull
impact-trace import facts/2026-04-30-A.jsonl
...
**상태:** 미구현, **추후 구현 검토**. content-addressable 모델이 이미 받아주는 구조라 *우선순위 결정 시 빠르게 진입 가능*. (reflective consolidation, branch GC) 이후 후보. 시작은 git 채널 + 매뉴얼 export/import → 검증되면 `impact-trace sync` 자동화 사이드카 추가 방향.
