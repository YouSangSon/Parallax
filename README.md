...
| **Analysis Trust** | indexed/skipped coverage, adapter confidence, known gap 수를 Impact Summary 안에서 바로 표시해 결과를 어디까지 믿어도 되는지 노출 |
...
Impact Map은 그래프 위에 ranked impact route card를 먼저 보여줘 “무엇을 먼저 봐야 하는지”를 바로 드러낸다. Impact Summary는 Analysis Trust를 함께 보여줘 skipped coverage, heuristic adapter, known gap을 숨기지 않는다. 각 노드는 영향 범주(`Runtime code`, `Tests to verify`, `Docs & policy`)와 confidence(`proven`, `heuristic`)를 노드 안에 직접 표시한다. 모바일에서는 SVG 라벨이 작아지는 문제를 줄이기 위해 같은 route card를 세로로 배치한다.
