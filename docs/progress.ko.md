...
업데이트: 2026-05-11 (OpenAPI contract impact baseline)
...
**Phase 6B multi-language + Spring Boot adapter pack v0 + trusted evidence + OpenAPI contract baseline**와
...
| Phase 6/7 | workspace/cross-repo resolver | contract baseline은 준비, 실제 workspace catalog loading, cross-repo provider/consumer resolver, contract diff는 예정 |
...
10. 실제 workspace catalog 파일을 정의하고 여러 repo를 등록/조회하는 CLI를 추가한다.
11. OpenAPI/protobuf/GraphQL/AsyncAPI contract baseline과 breaking-change 분류를 구현한다.
