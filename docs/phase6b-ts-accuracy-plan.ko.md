...
> **상태:** `/autoplan` + `/team-builder` 이후 사용자 stack 정정 반영. 파일명은 기존 링크 유지를 위해 유지한다.
...
지금 Impact Trace의 가장 큰 빈칸은 adapter framework가 아니라 **실제 사용 stack을 coverage gap 없이 설명하는 pragmatic semantic lane**이다. `SemanticAdapter`/`AdapterRun`/registry는 이미 있고, 기본 registry는 아직 `MultiLanguageRegexAdapter` 하나만 등록한다. 따라서 다음 작업은 TypeScript 하나를 깊게 파는 것이 아니라, 사용자의 실제 stack에 맞춰 v0 adapter pack을 만들고 Spring Boot를 first-class로 다루는 것이다.
...
- `TypeScriptCompilerAdapter` v0: `ts.createProgram` 기반 import/export/module relation
- Java/Kotlin parser v0: package/import/class/interface/function-ish declaration, Spring annotation relation
...
단, UI의 첫 버전에서 필요할 graph JSON contract와 evidence/resource shape는 이 phase의 output에서 깨지지 않게 둔다.
...
| Adapter | v0 minimum |
|---|---|
| TypeScript/JavaScript | imports/re-exports/dynamic import/require, exported declarations, test imports, TSX/JSX smoke |
...
현재 얇은 spine은 `bench/impact-bench.ts`와 `npm run bench`로 시작한다. report는 기본적으로 `.impact-trace/bench/impact-bench-report.json`에 쓰며, `.impact-trace/`는 gitignore 대상이라 반복 실행해도 작업트리를 오염시키지 않는다. 이 spine은 Phase 6B 전체 완료 선언이 아니라, 이후 Java/Kotlin/Spring Boot/Python/Go/Rust/TS/JS adapter depth pass가 개선됐는지 숫자로 확인하기 위한 기준선이다.
...
2. `test(bench): add ImpactBench multi-language fixtures`
   ...
   - regex baseline의 한계를 JSON report로 고정
