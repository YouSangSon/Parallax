...
| **Contract diff** | OpenAPI, GraphQL, Protobuf, AsyncAPI surface diff를 `breaking` / `non-breaking` / `unknown`으로 분류하고, OpenAPI response nullable addition, response format/enum change, request format addition/change, request enum-value removal도 감지 |
| **Consumer impact** | removed endpoint/operation, field removal/type/nullable/format/enum change, required request field 추가, request format addition/change, request enum-value removal 등을 known consumer와 연결 |
