...
The `query` subset supports a single optional relationship hop in either direction (`->` or `<-`), node labels, `WHERE` equality / `CONTAINS`, projection, and `LIMIT` — e.g. `MATCH (a)-[r:DEPENDS_ON]->(b) WHERE a.path CONTAINS 'store' RETURN a.path, b.path LIMIT 20`, or the reverse "what depends on X" form `MATCH (x)<-[r:DEPENDS_ON]-(d) WHERE x.path = 'src/store.ts' RETURN d.path`. Write, procedure, projection (`WITH`/`UNWIND`), and bidirectional clauses are rejected. `ingest-traces` is a write surface kept off the read-only MCP (invariant **I-8**); runtime observation only ever raises confidence.
