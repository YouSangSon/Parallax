...
> 마지막 업데이트: 2026-05-10 (Phase 6B ImpactBench + multi-language adapter pack v0 routing landed; agentmemory adoption boundary documented; source span/git snapshot depth pass next)
...
| `impact_trace_search_context` — keyword/path/symbol/relation/evidence 검색으로 agent가 파일을 덤프하지 않고 관련 entity를 찾는 read-only context discovery |
| `impact_trace_search_context` ranking v1 — SQLite FTS/BM25 + existing semantic recall + relation proximity를 RRF로 fuse |
