...
- Read-only graph query surface: a `parallax query "<cypher>"` command, a `parallax_query` MCP tool (`readOnlyHint: true`), and an `executeGraphQuery` API running a small, deterministic Cypher subset over the canonical entities/relations graph — single optional relationship hop, node labels, `WHERE` equality/`CONTAINS`, projection, and `LIMIT` — translated to parameterized SQL. Write, procedure, projection (`WITH`/`UNWIND`), and reverse-direction clauses are rejected with clear errors before any database access.
