...
| `parallax_query` | 在已索引的 entity/relation 图上运行只读 Cypher 子集（正向/反向/可变长度 hop、label、WHERE =/CONTAINS、投影、ORDER BY、LIMIT）并返回 JSON 行以及所查询的 index run 和可导航的 entity 资源 | 是 |
