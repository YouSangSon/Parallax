...
| next | protobuf/AsyncAPI consumer resolver |
...
| 5 | contract diff impact | 진행 중. workspace catalog, contract baseline, cross-repo links 위에서 OpenAPI endpoint/nested schema risk, Protobuf RPC/message field risk, GraphQL root/object/input schema risk, AsyncAPI operation/message payload risk를 만든다. GraphQL consumer resolver v0는 operation document top-level selection을 provider root field와 연결한다. 다음은 protobuf/AsyncAPI consumer resolver다. |
