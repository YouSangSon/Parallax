...
| `parallax_cross_repo_consumers` | provider service/contract/route의 consumer를 persisted workspace link에서 질의 | 예 |
| `parallax_cross_repo_providers` | consumer service/file이 사용하는 provider를 persisted workspace link에서 질의 | 예 |
| `parallax_resolve_cross_repo_contracts` | workspace link row를 저장하지 않고 cross-repo provider/consumer contract link를 미리보기 | 예 |
