...
| `impact_trace_search_context` — keyword/path/symbol/relation/evidence 검색으로 agent가 파일을 덤프하지 않고 관련 entity를 찾는 read-only context discovery. v0 landed |
...
| context access telemetry — context pack/resource fetch가 실제로 쓰인 정도를 기록해 context 절감 효과를 측정 |
